...
std::vector<Finding> collect_all_findings(const Snapshot& snapshot) {
    ...
    findings.reserve(snapshot.hooks.findings.size() + snapshot.maps.findings.size() +
                     snapshot.fd.findings.size() + snapshot.signal.findings.size() +
                     snapshot.vdso.findings.size() + snapshot.linker.findings.size());
    findings.insert(findings.end(), snapshot.hooks.findings.begin(), snapshot.hooks.findings.end());
    findings.insert(findings.end(), snapshot.maps.findings.begin(), snapshot.maps.findings.end());
    ...
    findings.insert(findings.end(), snapshot.vdso.findings.begin(), snapshot.vdso.findings.end());
...
std::string encode_snapshot(const Snapshot& snapshot) {
    ...
    output << "CRITICAL_COUNT=" << count_findings_by_severity(snapshot, FindingSeverity::kCritical)
           << '\n';
    ...
    for (const Finding& finding : collect_all_findings(snapshot)) {
        output << "FINDING=" << finding.section << '\t' << finding.category << '\t' << finding.label
               << '\t' << to_string(finding.severity) << '\t' << escape_value(finding.detail)
               << '\n';
    }
