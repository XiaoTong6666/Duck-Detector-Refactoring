...
MapEntry parse_map_line(const char* line) {
    ...
    int path_offset = 0;
    if (std::sscanf(line, "%lx-%lx %4s %*s %*s %lu %n", &start, &end, perms, &inode, &path_offset) <
        4) {
        return entry;
    }
...
bool contains_any(const std::string& value, const std::initializer_list<const char*> needles) {
...
bool is_known_app_code_cache_path(const std::string& lowered) {
    return starts_with(lowered, "/memfd:jit-cache") || starts_with(lowered, "/memfd:/jit-cache") ||
...
bool is_system_path(const std::string& path) {
    return path.rfind("/system/", 0) == 0 || path.rfind("/system_ext/", 0) == 0 ||
           path.rfind("/vendor/", 0) == 0 || path.rfind("/product/", 0) == 0 ||
           path.rfind("/odm/", 0) == 0 || path.rfind("/apex/", 0) == 0;
}
...
bool is_benign_art_code_cache_path(const std::string& path) {
    ...
    if ((starts_with(lowered, "dalvik-") || starts_with(lowered, "jit-") ||
         starts_with(lowered, "zygote-") || starts_with(lowered, "data-")) &&
        contains_known_art_code_cache_name(lowered)) {
        return true;
    }
    return false;
...
bool is_anonymous_path(const std::string& path) {
    return path.empty() || path == "[anon]" || path.rfind("[anon:", 0) == 0 ||
