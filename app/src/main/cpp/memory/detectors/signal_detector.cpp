...
Finding make_signal_finding(const char* label, const FindingSeverity severity,
...
SignalSignals detect_signal_anomalies(const std::vector<MapEntry>& maps) {
    ...
    for (const int signum : watched_signals) {
        ...
        if (handler == 0 || handler == reinterpret_cast<std::uintptr_t>(SIG_DFL) ||
        ...
        detail << signal_name(signum) << " handler @ 0x" << std::hex << handler << " in " << path;

        if (!entry.has_value() || is_anonymous_path(path)) {
            signals.suspicious_handler = true;
            signals.anonymous_handler = true;
            signals.findings.push_back(make_signal_finding("Anonymous signal handler",
        ...
        if (is_suspicious_loader_path(lowered) || lowered.find("frida") != std::string::npos) {
            ...
            signals.findings.push_back(make_signal_finding("Suspicious signal handler path",
                                                           FindingSeverity::kHigh, detail.str()));
        }
    }
