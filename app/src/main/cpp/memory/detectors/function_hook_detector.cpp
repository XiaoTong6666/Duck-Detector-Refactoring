...
std::vector<std::pair<std::uintptr_t, std::uintptr_t>> module_ranges(
    const std::vector<MapEntry>& maps, const std::vector<const char*>& needles) {
...
bool in_ranges(const std::vector<std::pair<std::uintptr_t, std::uintptr_t>>& ranges,
...
bool looks_like_branch_instruction(const std::uint8_t* bytes) {
    ...
    return (inst >> 26) == 0x05 || (inst >> 26) == 0x25 || (inst & 0xFFFFFC1F) == 0xD61F0000 ||
           (inst & 0xFFFFFC1F) == 0xD63F0000;
#elif defined(__x86_64__)
    return bytes[0] == 0xE9 || bytes[0] == 0xEB || (bytes[0] == 0xFF && bytes[1] == 0x25) ||
...
#if defined(__aarch64__)
    const std::uint32_t inst = *reinterpret_cast<const std::uint32_t*>(bytes);
    return (inst & 0xFFC003FF) == 0xA9BF7BFD || (inst & 0xFF8003FF) == 0xD10003FF ||
           inst == 0x910003FD || inst == 0xD503201F || inst == 0xD503233F ||
           (inst & 0x9F000000) == 0x90000000;
#elif defined(__x86_64__)
    return (bytes[0] == 0xF3 && bytes[1] == 0x0F && bytes[2] == 0x1E) || bytes[0] == 0x55 ||
...
bool looks_like_trampoline(const std::uint8_t* bytes, bool* suspicious_jump) {
...
Finding make_finding(const char* category, const char* label, const FindingSeverity severity,
...
HookSignals detect_function_hooks(const std::vector<MapEntry>& maps) {
    ...
    const std::array<FunctionTarget, 12> targets{{
        ...
        {"mmap", {"libc.so"}},
        {"mprotect", {"libc.so"}},
        {"dlopen", {"libdl.so", "linker64", "linker"}},
        {"dlsym", {"libdl.so", "linker64", "linker"}},
        {"dlclose", {"libdl.so", "linker64", "linker"}},
    }};
    ...
    for (const FunctionTarget& target : targets) {
        ...
        if (!expected_ranges.empty() && !in_ranges(expected_ranges, address)) {
            ...
            detail << target.name << " resolved to " << resolved_path << " @ 0x" << std::hex
                   << address;
            signals.findings.push_back(make_finding("GOT_PLT",
                                                    "Resolved address escaped expected module",
        ...
        if (looks_like_branch_instruction(bytes) && !looks_like_expected_prologue(bytes)) {
            ...
            detail << target.name << " starts with branch-like bytes [" << hex_prefix(bytes, 12)
                   << "]";
            signals.findings.push_back(make_finding("INLINE_HOOK", "Branch-like prologue",
                                                    FindingSeverity::kHigh, detail.str()));
        }
        ...
        if (looks_like_trampoline(bytes, &suspicious_jump)) {
            ...
            modified_functions.insert(target.name);
            std::ostringstream detail;
            detail << target.name << " shows a trampoline-style entry [" << hex_prefix(bytes, 12)
                   << "]";
            signals.findings.push_back(make_finding(
                "TRAMPOLINE",
                suspicious_jump ? "Trampoline with jump handoff" : "Trampoline-style entry",
                suspicious_jump ? FindingSeverity::kHigh : FindingSeverity::kMedium, detail.str()));
        }
    }
