...
Finding make_map_finding(const char* category, const char* label, const FindingSeverity severity,
...
MapsSignals detect_maps_anomalies(const std::vector<MapEntry>& maps,
    ...
    for (const MapEntry& entry : maps) {
        ...
        if (entry.writable) {
            ...
            detail << "Writable+executable mapping 0x" << std::hex << entry.start << "-0x"
                   << entry.end << " " << (entry.path.empty() ? "[anonymous]" : entry.path);
            signals.findings.push_back(make_map_finding("TEXT_SECTION",
        ...
        if (is_anonymous_path(entry.path)) {
            ...
            std::ostringstream detail;
            detail << "Executable anonymous mapping 0x" << std::hex << entry.start << "-0x"
                   << entry.end << " " << (entry.path.empty() ? "[anonymous]" : entry.path);
            signals.findings.push_back(make_map_finding("SMAPS", "Anonymous executable code",
                                                        FindingSeverity::kHigh, detail.str()));
        }
    }
    ...
    for (const SmapsEntry& entry : smaps) {
        ...
        if (entry.swap_kb > 0) {
            ...
            std::ostringstream detail;
            detail << entry.map.path << " has " << entry.swap_kb << " kB swapped executable pages";
            signals.findings.push_back(make_map_finding("SMAPS", "Swapped executable pages",
        ...
        if (entry.shared_dirty_kb > 0 && is_system_path(entry.map.path)) {
            ...
            signals.findings.push_back(make_map_finding("SMAPS", "Shared-dirty system code",
                                                        FindingSeverity::kHigh, detail.str()));
        }
