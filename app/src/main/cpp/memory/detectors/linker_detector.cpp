...
bool is_runtime_artifact_path(const std::string& lowered) {
    return lowered.find(".oat") != std::string::npos ||
           lowered.find(".odex") != std::string::npos || lowered.find(".vdex") != std::string::npos;
}

bool is_app_package_path(const std::string& lowered) {
    return lowered.rfind("/data/app/", 0) == 0 || lowered.rfind("/mnt/expand/", 0) == 0;
}
...
bool is_shared_object_path(const std::string& lowered) {
    return lowered.find(".so") != std::string::npos || is_normal_apk_library_path(lowered);
}
...
bool path_matches(const std::string& lhs, const std::string& rhs) {
    if (lhs == rhs) {
        return true;
    }
    ...
    return basename_of(normalize_library_name(lhs)) == basename_of(normalize_library_name(rhs));
}

bool range_overlaps(const std::uintptr_t start, const std::uintptr_t end, const MapEntry& entry) {
    return start < entry.end && end > entry.start;
}

bool is_object_covered_in_maps(const LinkerObject& object, const std::vector<MapEntry>& maps) {
...
LinkerSignals detect_linker_anomalies(const std::vector<MapEntry>& maps) {
    ...
    for (const MapEntry& entry : maps) {
        ...
        if (is_deleted_library_candidate(entry)) {
            ...
            if (deleted_reported.insert(dedupe_key).second) {
                ...
                signals.findings.push_back(Finding{
                    .section = "LINKER",
                    .category = "LINKER",
        ...
        [](dl_phdr_info* info, size_t, void* data) -> int {
            ...
            if (info->dlpi_name != nullptr && info->dlpi_name[0] != '\0') {
                ...
                for (ElfW(Half) index = 0; index < info->dlpi_phnum; ++index) {
                    ...
                    const auto start = static_cast<std::uintptr_t>(info->dlpi_addr + phdr.p_vaddr);
    ...
    for (const LinkerObject& linker_object : linker_objects) {
        ...
        if (covered_in_maps && !suspicious_path) {
            continue;
        }
        if (covered_in_maps && (is_system_path(lowered) || is_normal_apk_library_path(lowered) ||
