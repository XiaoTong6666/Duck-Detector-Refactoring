...
Finding make_fd_finding(const char* label, const FindingSeverity severity,
...
FdSignals detect_fd_anomalies(const std::vector<MapEntry>& maps) {
    ...
    for (const MapEntry& entry : maps) {
        ...
        if (lowered.rfind("/dev/ashmem", 0) == 0 && !is_benign_exec_ashmem_path(lowered)) {
            signals.exec_ashmem = true;
            if (dedupe.insert(entry.path).second) {
                signals.findings.push_back(make_fd_finding("Executable ashmem mapping",
                                                           FindingSeverity::kHigh, entry.path));
            }
        }
        if (lowered.rfind("/dev/zero", 0) == 0) {
            signals.dev_zero_exec = true;
            if (dedupe.insert(entry.path).second) {
                signals.findings.push_back(make_fd_finding("Executable /dev/zero mapping",
                                                           FindingSeverity::kHigh, entry.path));
            }
        }
        if (lowered.find("memfd:") != std::string::npos &&
            !is_benign_loader_artifact_path(lowered) && is_suspicious_loader_path(lowered)) {
            signals.suspicious_memfd = true;
            if (dedupe.insert(entry.path).second) {
                signals.findings.push_back(make_fd_finding("Suspicious executable memfd",
        ...
        if (lowered.find("(deleted)") != std::string::npos &&
            ...
            if (dedupe.insert(entry.path).second) {
                signals.findings.push_back(make_fd_finding("Deleted executable library",
                                                           FindingSeverity::kHigh, entry.path));
            }
        }
    }
    ...
    while (dirent* entry = ::readdir(dir)) {
        ...
        if (lowered.find("memfd:") != std::string::npos &&
            ...
            if (dedupe.insert(target).second) {
                ...
                signals.findings.push_back(make_fd_finding(
                    "Suspicious memfd handle", FindingSeverity::kCritical, detail.str()));
            }
        }
        if (lowered.find("(deleted)") != std::string::npos &&
            !is_benign_deleted_runtime_target(lowered) &&
            ...
            if (dedupe.insert(target).second) {
                std::ostringstream detail;
                detail << "fd " << entry->d_name << " -> " << target;
                signals.findings.push_back(make_fd_finding("Deleted shared object handle",
