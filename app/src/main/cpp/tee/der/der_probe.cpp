...
Tlv parse_tlv(const std::vector<std::uint8_t>& bytes, std::size_t offset) {
    ...
    if ((first_len & 0x80U) == 0U) {
        ...
        if (bytes_count == 0 || bytes_count > 4 || offset + 1 + bytes_count >= bytes.size()) {
            return tlv;
        }
        header += bytes_count;
...
bool walk_der_tree(const std::vector<std::uint8_t>& bytes, std::size_t start, std::size_t end,
                   int depth, DerSnapshot* snapshot) {
