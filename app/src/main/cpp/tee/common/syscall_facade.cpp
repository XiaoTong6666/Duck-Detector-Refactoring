...
extern "C" long tee_asm_syscall6(long number, long arg0, long arg1, long arg2, long arg3, long arg4,
                                 long arg5);
...
bool read_monotonic_via_result(const SyscallCallResult& call, const timespec& ts,
                               std::uint64_t* out_ns) {
...
SyscallCallResult invoke_syscall6(SyscallBackend backend, long number, long arg0, long arg1,
...
SyscallCallResult invoke_syscall3(SyscallBackend backend, long number, long arg0, long arg1,
                                  long arg2) {
...
SyscallCallResult invoke_open_readonly(SyscallBackend backend, const char* path) {
    switch (backend) {
        ...
        case SyscallBackend::Asm:
            ...
            return invoke_syscall6(backend, __NR_openat, AT_FDCWD, reinterpret_cast<long>(path),
...
SyscallCallResult invoke_ioctl(SyscallBackend backend, int fd, unsigned long request, void* arg) {
    switch (backend) {
        case SyscallBackend::Libc: {
            errno = 0;
        ...
        case SyscallBackend::Asm:
            ...
            return invoke_syscall3(backend, __NR_ioctl, fd, static_cast<long>(request),
    ...
    switch (backend) {
        ...
        case SyscallBackend::Asm:
            ...
            return read_monotonic_via_result(
                invoke_syscall3(backend, __NR_clock_gettime, CLOCK_MONOTONIC,
...
bool select_preferred_local_timer(const bool request_cpu0_affinity, LocalTimerSelection* out) {
    if (out == nullptr) {
        return false;
    }
...
long raw_syscall3(long number, long arg0, long arg1, long arg2) {
    const SyscallCallResult result = invoke_syscall3(
        backend_available(SyscallBackend::Asm) ? SyscallBackend::Asm : SyscallBackend::Syscall,
...
long raw_ioctl(int fd, unsigned long request, void* arg) {
    const SyscallCallResult result = invoke_ioctl(
        backend_available(SyscallBackend::Asm) ? SyscallBackend::Asm : SyscallBackend::Syscall, fd,
        request, arg);
    errno = result.error_number;
    return result.value;
}
