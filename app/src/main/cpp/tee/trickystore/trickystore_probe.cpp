...
ducktee::common::SyscallCallResult call_ioctl_backend(const ducktee::common::SyscallBackend backend,
                                                      const int fd, const unsigned long request,
                                                      void* arg) {
...
std::string format_backend_observation(const IoctlBackendObservation& observation) {
    ...
    builder << ducktee::common::backend_label(observation.backend) << "(ret=" << observation.result
            << ", errno=" << observation.error_number
            << ", version=" << observation.protocol_version << ")";
    return builder.str();
}

bool backend_samples_aligned(const IoctlBackendObservation& reference,
...
void prepare_honeypot_payload(std::uint8_t* write_buffer, binder_write_read* bwr,
...
bool collect_honeypot_backend_samples(const int binder_fd,
                                      const ducktee::common::LocalTimerSelection& timer,
                                      HoneypotTimingPath* path) {
    if (path == nullptr || !ducktee::common::backend_available(path->backend)) {
        return false;
    }
    ...
    for (int index = 0; index < kHoneypotIterations; ++index) {
        ...
        if (!ducktee::common::local_timer_now_ns(timer, &start)) {
            path->failure = std::string("Failed to read ") + timer.source_label + " before ioctl.";
            return false;
        }
        const auto result = call_ioctl_backend(path->backend, binder_fd, BINDER_WRITE_READ, &bwr);
        if (!result.available) {
        ...
        if (!ducktee::common::local_timer_now_ns(timer, &end)) {
            path->failure = std::string("Failed to read ") + timer.source_label + " after ioctl.";
            return false;
        }
...
HoneypotRunSummary analyze_honeypot_paths(const std::vector<HoneypotTimingPath>& paths) {
    ...
    const auto libc_it =
        std::find_if(paths.begin(), paths.end(), [](const HoneypotTimingPath& path) {
    ...
    summary.fastest_lower_median_ns = summary.lower_found ? fastest_lower_path->median_ns() : 0;

    if (!summary.libc_available || !summary.lower_found ||
    ...
    summary.ratio_percent = (summary.libc_median_ns * 100ULL) /
                            std::max<std::uint64_t>(1ULL, summary.fastest_lower_median_ns);
    summary.suspicious = summary.stable_lower_paths && summary.gap_ns > summary.noise_floor_ns &&
    ...
    for (std::size_t index = 0; index < rel_count; ++index) {
        ...
        if (got_value != real_ioctl) {
            ...
            builder << "libbinder.so ioctl GOT entry resolved to " << got_value
                    << " instead of libc ioctl " << real_ioctl << " (hook library: " << hook_library
                    << ").";
...
MethodSnapshot run_single_syscall_ioctl_mismatch_probe() {
    ...
    for (const auto backend : available_backends()) {
        ...
        const auto result = call_ioctl_backend(backend, binder_fd, BINDER_VERSION, &version);
        if (!result.available) {
            continue;
        }
    ...
    for (std::size_t index = 1; index < observations.size(); ++index) {
        ...
        snapshot.findings.push_back(builder.str());
        snapshot.detail =
            "Binder version query returned different results across libc/syscall/asm backends.";
        return snapshot;
    }
...
MethodSnapshot detect_syscall_ioctl_mismatch() {
    ...
    if (snapshot.detected) {
        builder << "Binder version ioctl diverged on " << hit_count << "/" << kRepeatedProbeAttempts
                << " backend-comparison probes.";
        snapshot.detail = builder.str();
    } else {
        builder << "Binder version ioctl stayed aligned across " << kRepeatedProbeAttempts
                << " backend-comparison probes";
        if (hit_count > 0) {
            builder << " (" << hit_count << "/" << kRepeatedProbeAttempts << " suspicious run).";
        } else {
            builder << ".";
        }
...
MethodSnapshot detect_ioctl_inline_hook() {
    ...
    if (fd >= 0) {
        ...
        if (read(fd, &ehdr, sizeof(ehdr)) == sizeof(ehdr) &&
            ...
            const auto phdr_bytes = sizeof(ElfWord_Phdr) * static_cast<std::size_t>(ehdr.e_phnum);
            if (read(fd, phdrs.data(), phdr_bytes) == static_cast<ssize_t>(phdr_bytes)) {
                const uintptr_t ioctl_va = reinterpret_cast<uintptr_t>(ioctl_addr) - lib_info.base;
                for (const auto& phdr : phdrs) {
                    if (phdr.p_type != PT_LOAD) {
                        continue;
                    }
                    if (ioctl_va >= phdr.p_vaddr && ioctl_va < phdr.p_vaddr + phdr.p_filesz) {
                        const uintptr_t file_offset = phdr.p_offset + (ioctl_va - phdr.p_vaddr);
                        ...
                        if (read(fd, disk_prologue, sizeof(disk_prologue)) ==
                            ...
                            if (!ducktee::common::bytes_equal(memory_prologue, disk_prologue,
    ...
    if (!snapshot.detected) {
        ...
        for (int index = 0; index < 4; ++index) {
            ...
            if ((word & 0xFC000000U) == 0x14000000U && index == 0) {
                ...
                const uintptr_t target = reinterpret_cast<uintptr_t>(&instructions[index]) +
                                         static_cast<uintptr_t>(imm26 * 4);
                Dl_info target_info{};
                if (dladdr(reinterpret_cast<void*>(target), &target_info) &&
                    ...
                    snapshot.findings.push_back(
                        "ioctl begins with an unconditional branch into another library.");
...
MethodSnapshot run_single_ioctl_honeypot_probe(const int attempt) {
    ...
    if (run.suspicious) {
        snapshot.detected = true;
        std::ostringstream builder;
        builder << "Keystore-style binder ioctl median timing diverged across redundant backends: "
                << run.path_summary << ". gap=" << run.gap_ns << "ns"
                << ", noise_floor=" << run.noise_floor_ns << "ns"
                << ", ratio=" << run.ratio_percent << "%"
                << ". timer=" << snapshot.timer_source << ", affinity=" << snapshot.affinity_status;
        ...
        snapshot.findings.push_back(builder.str());
        snapshot.detail =
            "Keystore-style binder honeypot found a libc-vs-lower-path timing anomaly.";
    } else {
        std::ostringstream builder;
        builder << "Keystore-style binder honeypot timing stayed within normal bounds across "
                   "redundant backends. "
                << run.path_summary << " gap=" << run.gap_ns << "ns"
                << ", noise_floor=" << run.noise_floor_ns << "ns"
                << ", ratio=" << run.ratio_percent << "%"
        ...
        if (!run.stable_lower_paths) {
            builder << " Lower-level syscall and asm paths were not stable enough to escalate.";
        }
        snapshot.detail = builder.str();
    }
    return snapshot;
...
MethodSnapshot detect_ioctl_honeypot() {
    ...
    for (int attempt = 0; attempt < kRepeatedProbeAttempts; ++attempt) {
        ...
        if (single.honeypot_median_gap_ns > 0 || single.honeypot_median_noise_floor_ns > 0 ||
...
ProbeSnapshot inspect_process() {
    ...
    if (!findings.empty()) {
        ...
        std::ostringstream builder;
        builder << got_result.detail << " | " << inline_result.detail << " | "
