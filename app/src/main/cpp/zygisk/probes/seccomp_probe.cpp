...
void sigsys_handler(int, siginfo_t* info, void*) {
...
ProbeResult collect_seccomp_probe() {
    ...
    if (packet.triggered_syscall != 0) {
        result.traces.push_back({
            ...
            "Seccomp trap",
            std::string("pthread_attr_setstacksize triggered the ") +
                syscall_name(packet.triggered_syscall) +
                " syscall under a seccomp trap, which should not happen for the stock libc fast "
                "path.",
        });
        result.hit_count = 1;
