...
bool suspicious_owner_path(const std::string& path) {
    ...
    return lower.find("linker") == std::string::npos && lower.find("libdl.so") == std::string::npos;
}

bool arm64_branch_leaves_owner(const MapEntry& owner, const void* symbol) {
    ...
    for (const auto& [name, address] : targets) {
        ...
        if (owner == nullptr || suspicious_owner_path(owner->path) ||
            ...
            result.traces.push_back({
                ...
                "Linker hook",
                std::string(name) + " resolves outside the expected linker image or immediately "
