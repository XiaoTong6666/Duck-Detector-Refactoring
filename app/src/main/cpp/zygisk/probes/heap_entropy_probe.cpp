...
ProbeResult collect_heap_entropy_probe() {
    ...
    const double ratio =
        allocated > 0U ? static_cast<double>(free_kept) / static_cast<double>(allocated) : 0.0;
    ...
    if (suspicious) {
        result.traces.push_back({
            ...
            "Heap entropy",
            "mallinfo2 reported " + format_bytes(allocated) + " allocated and " +
                format_bytes(free_kept) + " free-kept heap space (ratio " + std::to_string(ratio) +
                ").",
        });
        result.hit_count = 1;
