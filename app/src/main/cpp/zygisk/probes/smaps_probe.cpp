    ...
    for (const auto& line : lines) {
        if (line.find('-') != std::string::npos && line.find(':') == std::string::npos) {
            ...
            const int matched = std::sscanf(line.c_str(), "%llx-%llx %4s %llx %31s %llu %n", &start,
                                            &end, perms, &offset, dev, &inode, &path_offset);
            if (matched >= 6 && path_offset > 0 && path_offset < static_cast<int>(line.size())) {
        ...
        if (starts_with(line, "Private_Dirty:")) {
            private_dirty_kb =
                static_cast<size_t>(std::strtoul(trim_copy(line.substr(14)).c_str(), nullptr, 10));
        } else if (starts_with(line, "Shared_Dirty:")) {
            shared_dirty_kb =
                static_cast<size_t>(std::strtoul(trim_copy(line.substr(13)).c_str(), nullptr, 10));
        }
    }
    finalize_region();
