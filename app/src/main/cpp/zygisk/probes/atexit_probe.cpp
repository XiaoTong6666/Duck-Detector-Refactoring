...
ProbeResult collect_atexit_probe() {
    ...
    const auto* atexit_owner = find_map_for_address(maps, reinterpret_cast<uintptr_t>(cxa_atexit));
    const auto* finalize_owner =
        find_map_for_address(maps, reinterpret_cast<uintptr_t>(cxa_finalize));

    const bool atexit_drift =
        atexit_owner != nullptr && atexit_owner->path.find("libc.so") == std::string::npos;
    const bool finalize_drift =
        finalize_owner != nullptr && finalize_owner->path.find("libc.so") == std::string::npos;
    if (atexit_drift || finalize_drift) {
        std::string detail =
            "__cxa_atexit owner=" +
            std::string(atexit_owner != nullptr ? atexit_owner->path : "<unknown>") +
            ", __cxa_finalize owner=" +
            std::string(finalize_owner != nullptr ? finalize_owner->path : "<unknown>") + '.';
