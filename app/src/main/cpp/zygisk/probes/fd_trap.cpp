...
int verify_trap_fd(const int fd) {
    ...
    if (fstat(fd, &status) != 0) {
        if (errno == EBADF) {
            set_details(
                "fstat returned EBADF. The trap descriptor was already closed in the specialized "
                "child process.");
            return kFdTrapEbadf;
        }
        set_details("fstat failed for the trap descriptor: " + std::string(std::strerror(errno)));
    ...
    const std::string actual(buffer.data(), read_bytes > 0 ? static_cast<size_t>(read_bytes) : 0U);
    if (read_bytes != static_cast<ssize_t>(sizeof(kTrapSignature) - 1) ||
        actual != kTrapSignature) {
        set_details(
            "Trap descriptor content drifted after specialization. Expected signature did not "
            "survive intact.");
    ...
    if (link_length <= 0) {
        set_details(
            "Trap descriptor remained readable, but /proc/self/fd no longer exposed a stable "
            "target for it.");
        return kFdTrapProcfsAnomaly;
    }
