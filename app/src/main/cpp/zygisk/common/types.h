...
struct MapEntry {
    ...
    [[nodiscard]] bool private_mapping() const {
        return perms.find('p') != std::string::npos;
    }
...
inline bool starts_with(const std::string& value, const std::string_view prefix) {
...
inline std::vector<MapEntry> read_proc_maps(const char* path = "/proc/self/maps") {
    ...
    while (std::getline(input, line)) {
        ...
        char device[32] = {};
        int path_offset = 0;
        const int matched = std::sscanf(line.c_str(), "%llx-%llx %4s %llx %31s %llu %n", &start,
                                        &end, perms, &offset, device, &inode, &path_offset);
        if (matched < 6) {
            continue;
        }
        MapEntry entry;
...
inline const MapEntry* find_map_for_address(const std::vector<MapEntry>& maps,
...
inline bool is_suspicious_loader_keyword(const std::string& path) {
    const auto lower = lowercase_copy(path);
    return lower.find("zygisk") != std::string::npos || lower.find("magisk") != std::string::npos ||
           lower.find("riru") != std::string::npos || lower.find("rezygisk") != std::string::npos;
}
