...
std::vector<std::string> read_property_names(JNIEnv* env, jobjectArray property_names) {
    ...
    for (jsize index = 0; index < count; ++index) {
        auto* java_key = static_cast<jstring>(env->GetObjectArrayElement(property_names, index));
        if (java_key == nullptr) {
            continue;
        }
...
Java_com_eltavine_duckdetector_features_playintegrityfix_data_native_PlayIntegrityFixNativeBridge_nativeCollectSnapshot(
    ...
    return to_jstring(env, duckdetector::playintegrityfix::encode_snapshot(
