...
int capture_log_callback(int, const char* fmt, ...) {
    if (fmt == nullptr) {
        return 0;
    }
...
int capture_audit_callback(void* auditdata, security_class_t, char* msgbuf, size_t msgbufsize) {
    const char* marker = auditdata != nullptr ? static_cast<const char*>(auditdata) : "missing";
...
LoadedSymbols load_symbols() {
    ...
    if (symbols.set_callback != nullptr && symbols.get_callback != nullptr &&
...
AuditProbeSnapshot collect_audit_snapshot() {
    ...
    for (const ProbeSpec& spec : kProbeSpecs) {
        ...
        const int result =
            symbols.check_access(source_context.c_str(), spec.tcontext, spec.tclass_name,
                                 spec.permission, snapshot.probe_marker.data());
        if (result == 0) {
    ...
    if (!snapshot.allow_observed && !snapshot.denial_observed && snapshot.callback_lines.empty() &&
        snapshot.failure_reason.empty()) {
        snapshot.failure_reason =
            "Controlled libselinux access checks produced no observable result.";
    }
