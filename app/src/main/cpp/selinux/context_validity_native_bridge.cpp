...
std::string encode_snapshot(const duckdetector::selinux::ContextValidityProbeSnapshot& snapshot) {
    ...
    if (snapshot.negative_control_rejected.has_value()) {
        output << "NEGATIVE_CONTROL_REJECTED=" << (*snapshot.negative_control_rejected ? '1' : '0')
               << '\n';
    }
    ...
    output << "DIRTY_POLICY_PROBE_ATTEMPTED=" << (snapshot.dirty_policy.probe_attempted ? '1' : '0')
           << '\n';
    ...
    output << "DIRTY_POLICY_CONTROLS_PASSED=" << (snapshot.dirty_policy.controls_passed ? '1' : '0')
           << '\n';
    output << "DIRTY_POLICY_STABLE=" << (snapshot.dirty_policy.stable ? '1' : '0') << '\n';
    if (!snapshot.dirty_policy.query_method.empty()) {
        output << "DIRTY_POLICY_QUERY_METHOD=" << escape_value(snapshot.dirty_policy.query_method)
               << '\n';
    }
    ...
    if (snapshot.dirty_policy.system_server_droidspacesd_binder_call_allowed.has_value()) {
        output << "DIRTY_POLICY_SYSTEM_SERVER_DROIDSPACESD_BINDER_CALL_ALLOWED="
               << (*snapshot.dirty_policy.system_server_droidspacesd_binder_call_allowed ? '1'
                                                                                         : '0')
               << '\n';
    }
...
std::string fallback_snapshot_payload(const std::string& reason) {
    try {
        ...
        snapshot.dirty_policy.query_method = "android.os.SELinux.checkSELinuxAccess";
...
Java_com_eltavine_duckdetector_features_selinux_data_native_SelinuxContextValidityBridge_nativeCollectContextValiditySnapshotInternal(
    ...
    try {
        return to_jstring(
            env, encode_snapshot(duckdetector::selinux::collect_context_validity_snapshot(env)));
