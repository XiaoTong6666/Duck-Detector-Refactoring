...
using SelinuxCheckAccessFn = int (*)(const char* scon, const char* tcon, const char* tclass,
...
template <typename T>
T resolve_symbol(void* handle, const char* symbol) {
    return reinterpret_cast<T>(dlsym(handle, symbol));
}

LoadedSelinuxSymbols load_selinux_symbols() {
    LoadedSelinuxSymbols symbols;
    symbols.is_selinux_enabled =
        resolve_symbol<IsSelinuxEnabledFn>(RTLD_DEFAULT, "is_selinux_enabled");
    symbols.security_getenforce =
        resolve_symbol<SecurityGetEnforceFn>(RTLD_DEFAULT, "security_getenforce");
    ...
    symbols.check_access =
        resolve_symbol<SelinuxCheckAccessFn>(RTLD_DEFAULT, "selinux_check_access");
    symbols.set_callback =
        resolve_symbol<SelinuxSetCallbackFn>(RTLD_DEFAULT, "selinux_set_callback");
    symbols.get_callback =
        resolve_symbol<SelinuxGetCallbackFn>(RTLD_DEFAULT, "selinux_get_callback");

#ifdef RTLD_NOLOAD
    if (symbols.is_selinux_enabled == nullptr || symbols.security_getenforce == nullptr ||
        symbols.getcon == nullptr || symbols.getpidcon == nullptr ||
        symbols.getfilecon == nullptr || symbols.freecon == nullptr ||
        symbols.check_access == nullptr || symbols.set_callback == nullptr ||
        ...
        if (symbols.handle != nullptr) {
            ...
            if (symbols.is_selinux_enabled == nullptr) {
                symbols.is_selinux_enabled =
                    resolve_symbol<IsSelinuxEnabledFn>(symbols.handle, "is_selinux_enabled");
            }
            if (symbols.security_getenforce == nullptr) {
                symbols.security_getenforce =
                    resolve_symbol<SecurityGetEnforceFn>(symbols.handle, "security_getenforce");
            }
            ...
            if (symbols.check_access == nullptr) {
                symbols.check_access =
                    resolve_symbol<SelinuxCheckAccessFn>(symbols.handle, "selinux_check_access");
            }
            if (symbols.set_callback == nullptr) {
                symbols.set_callback =
                    resolve_symbol<SelinuxSetCallbackFn>(symbols.handle, "selinux_set_callback");
            }
            if (symbols.get_callback == nullptr) {
                symbols.get_callback =
                    resolve_symbol<SelinuxGetCallbackFn>(symbols.handle, "selinux_get_callback");
            }
        }
    }
#endif
...
JavaSelinuxAccess resolve_java_selinux_access(JNIEnv* env) {
    ...
    access.check_access = env->GetStaticMethodID(
        access.selinux_class, "checkSELinuxAccess",
...
void release_java_selinux_access(JNIEnv* env, JavaSelinuxAccess& access) {
...
int dirty_policy_audit_callback(void* auditdata, security_class_t, char* msgbuf,
                                size_t msgbufsize) {
    const char* marker =
...
class ScopedDirtyPolicyAuditMarker {
   public:
    explicit ScopedDirtyPolicyAuditMarker(const LoadedSelinuxSymbols& symbols) : symbols_(symbols) {
...
std::optional<std::string> call_context_getter(const LoadedSelinuxSymbols& symbols,
                                               int (*getter)(char**)) {
...
std::optional<std::string> call_pid_context_getter(const LoadedSelinuxSymbols& symbols, pid_t pid) {
...
std::optional<std::string> call_file_context_getter(const LoadedSelinuxSymbols& symbols,
                                                    const char* path) {
...
void append_boolean_note(ContextValidityProbeSnapshot& snapshot, const char* label,
...
bool stable_result(const ContextCheckResult& first, const ContextCheckResult& second) {
...
bool pair_matches_expected(const ControlPairResult& pair, const bool expected_valid) {
    return pair.stable && pair.first.valid.has_value() && *pair.first.valid == expected_valid;
}
...
std::optional<bool> check_access_rule(const LoadedSelinuxSymbols& symbols, const char* source,
                                      const char* target, const char* target_class,
                                      const char* permission, void* auditdata = nullptr) {
    if (symbols.check_access != nullptr) {
        errno = 0;
        const int result =
            symbols.check_access(source, target, target_class, permission, auditdata);
...
std::optional<bool> check_java_access_rule(JNIEnv* env, const JavaSelinuxAccess& java_access,
    ...
    const jboolean result =
        env->CallStaticBooleanMethod(java_access.selinux_class, java_access.check_access,
                                     source_string, target_string, class_string, permission_string);
...
AccessPairResult check_access_rule_pair(const LoadedSelinuxSymbols& symbols, const char* source,
                                        const char* target, const char* target_class,
                                        const char* permission, void* auditdata = nullptr) {
...
AccessPairResult check_java_access_rule_pair(JNIEnv* env, const JavaSelinuxAccess& java_access,
                                             const char* source, const char* target,
                                             const char* target_class, const char* permission) {
    AccessPairResult result;
    result.first =
        check_java_access_rule(env, java_access, source, target, target_class, permission);
    result.second =
        check_java_access_rule(env, java_access, source, target, target_class, permission);
    result.stable = result.first.has_value() == result.second.has_value() &&
                    (!result.first.has_value() || *result.first == *result.second);
...
void append_access_note(DirtyPolicyProbeSnapshot& snapshot, const char* label,
...
DirtyPolicyProbeSnapshot collect_dirty_policy_snapshot(
    const LoadedSelinuxSymbols& symbols, JNIEnv* env, const JavaSelinuxAccess& java_access,
    const std::string& carrier_context, const bool carrier_matches_expected,
    ...
    if (!carrier_matches_expected || carrier_context.rfind(kExpectedCarrierPrefix, 0) != 0) {
        ...
        snapshot.notes.push_back(
            "Dirty policy access checks require the dedicated app_zygote carrier.");
        return snapshot;
    }
    if (dyntransition_check_passed.has_value() && !*dyntransition_check_passed) {
        ...
        snapshot.notes.push_back(
            "Dirty policy access checks were skipped because the carrier could not confirm "
            "app_zygote -> isolated_app dyntransition.");
        return snapshot;
    }
    ...
    if (audit_marker.installed()) {
        snapshot.notes.push_back(std::string("Audit marker: ") + kProbeMarkerPrefix +
                                 audit_marker.marker());
    } else {
        snapshot.notes.push_back(
    ...
    const AccessPairResult access_control =
        check_access_rule_pair(symbols, kExpectedCarrierPrefix, kIsolatedAppContext, kProcessClass,
                               kDyntransitionPermission, auditdata);
    const AccessPairResult negative_control =
        check_access_rule_pair(symbols, kUntrustedAppContext, kDirtyPolicyNegativeControlContext,
                               kBinderClass, kCallPermission, auditdata);

    snapshot.access_control_allowed = pair_allowed_value(access_control);
    if (const auto negative_allowed = pair_allowed_value(negative_control);
        negative_allowed.has_value()) {
    ...
    const AccessPairResult system_server_execmem =
        check_access_rule_pair(symbols, kSystemServerContext, kSystemServerContext, kProcessClass,
                               kExecmemPermission, auditdata);
    const AccessPairResult fsck_sys_admin =
        check_access_rule_pair(symbols, kFsckUntrustedContext, kFsckUntrustedContext,
                               kCapabilityClass, kSysAdminPermission, auditdata);
    const AccessPairResult shell_su_transition = check_access_rule_pair(
        symbols, kShellContext, kSuContext, kProcessClass, kTransitionPermission, auditdata);
    const AccessPairResult adbd_adbroot_binder_call = check_access_rule_pair(
        symbols, kAdbdContext, kAdbrootContext, kBinderClass, kCallPermission, auditdata);
    const AccessPairResult magisk_binder_call = check_access_rule_pair(
        symbols, kUntrustedAppContext, kMagiskContext, kBinderClass, kCallPermission, auditdata);
    const AccessPairResult ksu_file_read = check_access_rule_pair(
        symbols, kUntrustedAppContext, kKsuFileContext, kFileClass, kReadPermission, auditdata);
    const AccessPairResult lsposed_file_read = check_access_rule_pair(
        symbols, kUntrustedAppContext, kLsposedFileContext, kFileClass, kReadPermission, auditdata);
    const AccessPairResult magisk_droidspacesd_transition =
        check_access_rule_pair(symbols, kMagiskContext, kDroidspacesdContext, kProcessClass,
                               kDyntransitionPermission, auditdata);
    const AccessPairResult su_droidspacesd_transition =
        check_access_rule_pair(symbols, kSuContext, kDroidspacesdContext, kProcessClass,
                               kDyntransitionPermission, auditdata);
    const AccessPairResult system_server_droidspacesd_binder_call =
        check_access_rule_pair(symbols, kSystemServerContext, kDroidspacesdContext, kBinderClass,
                               kCallPermission, auditdata);
    const AccessPairResult msd_app_daemon_connect =
        check_access_rule_pair(symbols, kMsdAppContext, kMsdDaemonContext, kUnixStreamSocketClass,
                               kConnectToPermission, auditdata);
    const AccessPairResult msd_daemon_self_connect =
        check_access_rule_pair(symbols, kMsdDaemonContext, kMsdDaemonContext,
                               kUnixStreamSocketClass, kConnectToPermission, auditdata);
    const AccessPairResult msd_daemon_selinuxfs_read = check_access_rule_pair(
        symbols, kMsdDaemonContext, kSelinuxfsContext, kFileClass, kReadPermission, auditdata);
    const AccessPairResult msd_daemon_configfs_dir_search = check_access_rule_pair(
        symbols, kMsdDaemonContext, kConfigfsContext, kDirClass, kSearchPermission, auditdata);
    const AccessPairResult msd_daemon_configfs_file_write = check_access_rule_pair(
        symbols, kMsdDaemonContext, kConfigfsContext, kFileClass, kWritePermission, auditdata);
    const AccessPairResult xposed_data_file_read = check_access_rule_pair(
        symbols, kUntrustedAppContext, kXposedDataContext, kFileClass, kReadPermission, auditdata);
    const AccessPairResult zygote_adb_data_search = check_access_rule_pair(
        symbols, kZygoteContext, kAdbDataFileContext, kDirClass, kSearchPermission, auditdata);
    ...
    snapshot.magisk_droidspacesd_transition_allowed =
        pair_allowed_value(magisk_droidspacesd_transition);
    snapshot.su_droidspacesd_transition_allowed = pair_allowed_value(su_droidspacesd_transition);
    snapshot.system_server_droidspacesd_binder_call_allowed =
        pair_allowed_value(system_server_droidspacesd_binder_call);
    ...
    snapshot.msd_daemon_configfs_dir_search_allowed =
        pair_allowed_value(msd_daemon_configfs_dir_search);
    snapshot.msd_daemon_configfs_file_write_allowed =
        pair_allowed_value(msd_daemon_configfs_file_write);
    ...
    snapshot.stable =
        snapshot.stable && system_server_execmem.stable && fsck_sys_admin.stable &&
        adbd_adbroot_binder_call.stable && magisk_binder_call.stable && ksu_file_read.stable &&
        lsposed_file_read.stable && magisk_droidspacesd_transition.stable &&
        su_droidspacesd_transition.stable && system_server_droidspacesd_binder_call.stable &&
        msd_app_daemon_connect.stable && msd_daemon_self_connect.stable &&
        msd_daemon_selinuxfs_read.stable && msd_daemon_configfs_dir_search.stable &&
        msd_daemon_configfs_file_write.stable && xposed_data_file_read.stable &&
        zygote_adb_data_search.stable && (!is_user_build() || shell_su_transition.stable);
    ...
    if (is_user_build()) {
        ...
        snapshot.notes.push_back(
            "shell -> su transition skipped because ro.build.type is not user.");
    }
    ...
    append_access_note(snapshot, "magisk -> droidspacesd dyntransition",
                       magisk_droidspacesd_transition);
    append_access_note(snapshot, "su -> droidspacesd dyntransition", su_droidspacesd_transition);
    append_access_note(snapshot, "system_server -> droidspacesd binder",
                       system_server_droidspacesd_binder_call);
    ...
    append_access_note(snapshot, "msd_daemon -> configfs dir search",
                       msd_daemon_configfs_dir_search);
    append_access_note(snapshot, "msd_daemon -> configfs file write",
                       msd_daemon_configfs_file_write);
...
void append_repeat_note(ContextValidityProbeSnapshot& snapshot, const char* label,
...
ContextCheckResult check_context_validity(const char* context) {
    ...
    if (fd < 0) {
        ...
        if (error == EINVAL) {
            result.valid = false;
            result.note =
                std::string("Invalid context: ") + context + " errno=" + std::to_string(error);
        } else {
            result.note =
                std::string("Unavailable: ") + context + " errno=" + std::to_string(error);
        }
        return result;
    }
    ...
    if (error == EINVAL) {
        ...
        result.note = std::string("Unavailable: ") + context + " errno=" + std::to_string(error);
    }
    return result;
}
...
ContextValidityProbeSnapshot collect_context_validity_snapshot(JNIEnv* env) {
    ...
    snapshot.dyntransition_check_passed =
        check_access_rule(symbols, kExpectedCarrierPrefix, kIsolatedAppContext, kProcessClass,
                          kDyntransitionPermission);
    snapshot.dirty_policy = collect_dirty_policy_snapshot(
        symbols, env, java_access, carrier_context, snapshot.carrier_matches_expected,
        snapshot.dyntransition_check_passed);
    ...
    if (snapshot.dyntransition_check_passed.has_value() && !*snapshot.dyntransition_check_passed) {
        snapshot.failure_reason = "app_zygote dyntransition self-check failed.";
        append_note(snapshot,
    ...
    snapshot.oracle_controls_passed = pair_matches_expected(carrier_control, true) &&
    ...
    append_boolean_note(snapshot, "File negative control rejected",
    ...
    if (!snapshot.ksu_results_stable) {
        ...
        append_note(snapshot,
                    "The KSU-specific context verdict changed across repeated writes, so it was "
                    "not trusted.");
        release_java_selinux_access(env, java_access);
        if (symbols.owns_handle && symbols.handle != nullptr) {
            dlclose(symbols.handle);
