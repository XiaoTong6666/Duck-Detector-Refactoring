...
constexpr const char* kSuspiciousTokens[] = {
    "magisk", "apatch", "kernelsu", ":su:", "permissive",
};
...
bool contains_any_token(const std::string& value, const char* const* tokens, size_t token_count) {
...
bool is_suspicious_context(const std::string& context) {
    return !context.empty() &&
           contains_any_token(context, kSuspiciousTokens,
                              sizeof(kSuspiciousTokens) / sizeof(kSuspiciousTokens[0]));
}

bool is_normal_context(const std::string& context) {
    return !context.empty() && contains_any_token(context, kNormalTokens,
...
std::string build_snapshot_payload() {
    ...
    const int proc_fd = syscall_openat_readonly(kProcDirPath, O_RDONLY | O_DIRECTORY | O_CLOEXEC);
    if (proc_fd >= 0) {
                ...
                if (is_process_dir) {
                    ...
                    if (!context.empty()) {
                        ...
                        if (is_suspicious_context(context)) {
                            suspicious_processes.emplace_back(std::string("PID ") + entry->d_name +
                                                              ": " + context);
                        }
                    } else {
                        ++denied_processes;
