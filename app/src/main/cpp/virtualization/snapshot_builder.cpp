...
std::string mount_anchor_key(const std::string& mount_id, const std::string& major_minor,
                             const std::string& root, const std::string& mount_point,
                             const std::string& fs_type, const std::string& source) {
    return mount_id + "|" + major_minor + "|" + root + "|" + mount_point + "|" + fs_type + "|" +
           source;
}

void add_finding(Snapshot& snapshot, std::set<std::string>& dedupe, const std::string& group,
                 const std::string& severity, const std::string& label, const std::string& value,
                 const std::string& detail) {
    const std::string key = group + "|" + severity + "|" + label + "|" + value + "|" + detail;
...
void scan_properties(Snapshot& snapshot, std::set<std::string>& dedupe) {
    ...
    if (roKernelQemu == "1") {
        add_finding(snapshot, dedupe, "ENVIRONMENT", "DANGER", "ro.kernel.qemu", "Guest",
    ...
    if ((!roBootQemu.empty() && roBootQemu != "0") || !avdName.empty() || !qemuLcd.empty()) {
        ...
        add_finding(snapshot, dedupe, "ENVIRONMENT", "DANGER", "QEMU guest properties", "Present",
                    detail.str());
    }
    ...
    if (has_hardware_cluster) {
        add_finding(snapshot, dedupe, "ENVIRONMENT", "DANGER", "Emulator hardware props",
                    "goldfish/ranchu", hardware_detail.str());
    }

    const std::string nativeBridge = read_property("ro.dalvik.vm.native.bridge");
    if (!nativeBridge.empty() && nativeBridge != "0") {
        add_finding(snapshot, dedupe, "TRANSLATION", "WARNING", "ro.dalvik.vm.native.bridge",
                    nativeBridge, "ART native bridge is configured for translated execution.");
    }

    const std::string hypervisorSupported = read_property("ro.boot.hypervisor.vm.supported");
    if (!hypervisorSupported.empty() && hypervisorSupported != "0") {
        add_finding(
            snapshot, dedupe, "ENVIRONMENT", "INFO", "Hypervisor capability", hypervisorSupported,
...
void scan_device_nodes(Snapshot& snapshot, std::set<std::string>& dedupe) {
    ...
    for (const auto* node : nodes) {
        if (file_exists(node)) {
            add_finding(snapshot, dedupe, "RUNTIME", "DANGER", "Emulator device node", node,
...
void scan_maps(Snapshot& snapshot, std::set<std::string>& dedupe) {
    ...
    while (std::getline(input, line)) {
        ...
        for (const auto& token : translation_tokens) {
            if (line.find(token) != std::string::npos) {
                add_finding(snapshot, dedupe, "TRANSLATION", "WARNING",
                            "Mapped translation library", token, line);
            }
        }

        if (line.find("goldfish") != std::string::npos ||
            line.find("ranchu") != std::string::npos || line.find("qemu") != std::string::npos) {
            add_finding(snapshot, dedupe, "RUNTIME", "DANGER", "Mapped emulator library", "Present",
                        line);
        }

        if (line.find("authfs") != std::string::npos)
            avf_tokens.insert("authfs");
        if (line.find("virtiofs") != std::string::npos)
            avf_tokens.insert("virtiofs");
        if (line.find("crosvm") != std::string::npos)
            avf_tokens.insert("crosvm");
        if (line.find("microdroid") != std::string::npos)
            avf_tokens.insert("microdroid");
    }
    ...
    if (avf_tokens.size() >= 2) {
        ...
        add_finding(snapshot, dedupe, "RUNTIME", "DANGER", "AVF runtime", joined.str(),
...
void scan_mountinfo(Snapshot& snapshot, std::set<std::string>& dedupe) {
    ...
    while (std::getline(input, line)) {
        ...
        const std::string anchor_key =
            mount_anchor_key(mount_id, major_minor, root, mount_point, fs_type, source);
        if (mount_point == "/apex")
            snapshot.apexMountKey = anchor_key;
        if (mount_point == "/system")
            snapshot.systemMountKey = anchor_key;
        if (mount_point == "/vendor")
            snapshot.vendorMountKey = anchor_key;

        if ((mount_point == "/apex" || mount_point == "/system" || mount_point == "/vendor") &&
            contains_token(fs_type + " " + source,
                           {"authfs", "virtiofs", "microdroid", "crosvm"})) {
            add_finding(snapshot, dedupe, "RUNTIME", "DANGER", "Mount anchor artifact", mount_point,
                        anchor_key);
        }
    }

    if (authfs_seen) {
        add_finding(snapshot, dedupe, "RUNTIME", "DANGER", "authfs runtime", "Present",
                    "authfs mount was visible from /proc/self/mountinfo.");
    }
    if (authfs_seen && avf_other_seen) {
        add_finding(
            snapshot, dedupe, "RUNTIME", "DANGER", "AVF runtime", "Present",
    ...
    while ((entry = readdir(dir)) != nullptr) {
        ...
        for (const auto& token : translation_tokens) {
            if (target.find(token) != std::string::npos) {
                add_finding(snapshot, dedupe, "TRANSLATION", "WARNING",
                            "FD target translation residue", token, target);
            }
        }

        if (contains_token(target,
                           {"qemu", "goldfish", "ranchu", "authfs", "crosvm", "microdroid"})) {
            add_finding(snapshot, dedupe, "RUNTIME", "DANGER", "FD target runtime artifact",
                        "Present", target);
        }
    }
    closedir(dir);
...
void scan_cmdline(Snapshot& snapshot, std::set<std::string>& dedupe) {
    ...
    if (cmdline.find("-Xnative-bridge") != std::string::npos) {
        add_finding(snapshot, dedupe, "TRANSLATION", "WARNING", "Cmdline native bridge", "Present",
                    cmdline);
    }
}
...
void scan_renderer(Snapshot& snapshot, std::set<std::string>& dedupe) {
    ...
    const std::string lowered =
        lowercase_copy(renderer.renderer + " " + renderer.vendor + " " + renderer.version);
    if (contains_token(lowered, {"android emulator opengl es translator", "gfxstream", "virgl",
                                 "virtio_gpu", "crosvm"})) {
        add_finding(snapshot, dedupe, "RUNTIME", "WARNING", "Graphics renderer",
                    renderer.renderer.empty() ? "Detected" : renderer.renderer,
                    renderer.vendor + "\n" + renderer.renderer + "\n" + renderer.version);
    } else if (contains_token(lowered, {"swiftshader"})) {
        add_finding(snapshot, dedupe, "RUNTIME", "INFO", "Graphics renderer",
...
std::string encode_snapshot(const Snapshot& snapshot) {
    ...
    for (const auto& finding : snapshot.findings) {
        output << "FINDING=" << finding.group << '\t' << finding.severity << '\t' << finding.label
               << '\t' << finding.value << '\t' << encode_value(finding.detail) << '\n';
    }
    return output.str();
}
