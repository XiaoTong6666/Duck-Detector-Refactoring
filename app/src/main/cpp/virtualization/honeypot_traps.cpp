...
std::string encode_basic_pack(bool available, bool supported, bool disabled,
...
long asm_syscall6(long number, long arg0, long arg1, long arg2, long arg3, long arg4, long arg5,
                  int* outErrno) {
...
bool run_syscall_item(const std::string& label, Callable callable,
    ...
    for (int attempt = 0; attempt < 3; ++attempt) {
        ...
        const bool returnMismatch =
            triplet.libcRet != triplet.rawRet || triplet.libcRet != triplet.asmRet ||
            triplet.libcErrno != triplet.rawErrno || triplet.libcErrno != triplet.asmErrno;
        const long long fastest = std::max(
            1LL,
            std::min(triplet.libcElapsedNs, std::min(triplet.rawElapsedNs, triplet.asmElapsedNs)));
        const long long slowest =
            std::max(triplet.libcElapsedNs, std::max(triplet.rawElapsedNs, triplet.asmElapsedNs));
        ...
        detail << "libc_ret=" << triplet.libcRet << " libc_errno=" << triplet.libcErrno
               ...
               << " libc_ns=" << triplet.libcElapsedNs << " raw_ns=" << triplet.rawElapsedNs
...
TrapResult run_syscall_parity_trap() {
    ...
    for (int attempt = 0; attempt < 3; ++attempt) {
        ...
        const long rawRet =
            syscall(__NR_openat, AT_FDCWD, path.str().c_str(), O_RDONLY | O_CLOEXEC, 0);
        ...
        std::ostringstream detail;
        detail << "libc_ret=" << libcRet << " libc_errno=" << libcErrno << " raw_ret=" << rawRet
               << " raw_errno=" << rawErrno << " libc_ns=" << libcElapsed
               << " raw_ns=" << rawElapsed;
        result.attempts.push_back(TrapAttempt{suspicious, detail.str()});
    }
    return finalize_result(
        result, "Compares libc and raw openat error paths for a transient missing-path trap.");
}
...
TrapResult run_asm_counter_trap() {
    ...
    for (int attempt = 0; attempt < 3; ++attempt) {
        ...
        const unsigned long long counterDelta =
            counterEnd > counterStart ? (counterEnd - counterStart) : 0ULL;
        const long long wallDelta = wallEnd - wallStart;
        const long long counterNs =
            (freq > 0ULL) ? static_cast<long long>((counterDelta * 1000000000ULL) / freq) : 0LL;
    ...
    return finalize_result(
        result, "Correlates arm64 cntvct or cntfrq readings with wall-clock work windows.");
#else
...
std::string run_sacrificial_syscall_pack() {
    ...
    if (g_sacrificialSyscallPackDisabled) {
        return encode_basic_pack(true, false, true,
                                 "Sacrificial syscall pack was disabled after a previous SIGSYS in "
                                 "this helper process.");
    }

    int pipefd[2] = {-1, -1};
    if (pipe(pipefd) != 0) {
        return encode_basic_pack(true, false, false,
                                 "Failed to create pipe for sacrificial syscall pack.");
    }
    ...
    if (child == 0) {
        ...
        auto call_via_syscall = [](long number, long arg0, long arg1, long arg2, long arg3,
                                   long arg4, long arg5, int* outErrno) -> long {
            errno = 0;
        ...
        const bool openat2Supported = run_syscall_item(
            "openat2",
            [&](int attempt) {
                const std::string path =
                    "/proc/self/virtualization_openat2_missing_" + std::to_string(attempt);
                struct open_how how{};
                how.flags = static_cast<std::uint64_t>(O_RDONLY | O_CLOEXEC);
                SyscallAttemptTriplet triplet;
                const long long libcStart = monotonic_ns();
                triplet.libcRet = call_via_syscall(
                    __NR_openat2, AT_FDCWD, reinterpret_cast<long>(path.c_str()),
                    reinterpret_cast<long>(&how), sizeof(how), 0, 0, &triplet.libcErrno);
                triplet.libcElapsedNs = monotonic_ns() - libcStart;
                const long long rawStart = monotonic_ns();
                triplet.rawRet = call_via_syscall(
                    __NR_openat2, AT_FDCWD, reinterpret_cast<long>(path.c_str()),
                    reinterpret_cast<long>(&how), sizeof(how), 0, 0, &triplet.rawErrno);
                triplet.rawElapsedNs = monotonic_ns() - rawStart;
                const long long asmStart = monotonic_ns();
                triplet.asmRet = asm_syscall6(
                    __NR_openat2, AT_FDCWD, reinterpret_cast<long>(path.c_str()),
                    reinterpret_cast<long>(&how), sizeof(how), 0, 0, &triplet.asmErrno);
        ...
        const bool statxSupported = run_syscall_item(
            "statx",
            [&](int attempt) {
                const std::string path =
                    "/proc/self/virtualization_statx_missing_" + std::to_string(attempt);
                struct statx statxBuffer{};
                SyscallAttemptTriplet triplet;
                const long long libcStart = monotonic_ns();
                triplet.libcRet = call_via_syscall(
                    __NR_statx, AT_FDCWD, reinterpret_cast<long>(path.c_str()), 0,
                    STATX_BASIC_STATS, reinterpret_cast<long>(&statxBuffer), 0, &triplet.libcErrno);
                triplet.libcElapsedNs = monotonic_ns() - libcStart;
                const long long rawStart = monotonic_ns();
                triplet.rawRet = call_via_syscall(
                    __NR_statx, AT_FDCWD, reinterpret_cast<long>(path.c_str()), 0,
                    STATX_BASIC_STATS, reinterpret_cast<long>(&statxBuffer), 0, &triplet.rawErrno);
                triplet.rawElapsedNs = monotonic_ns() - rawStart;
                const long long asmStart = monotonic_ns();
                triplet.asmRet = asm_syscall6(
                    __NR_statx, AT_FDCWD, reinterpret_cast<long>(path.c_str()), 0,
                    STATX_BASIC_STATS, reinterpret_cast<long>(&statxBuffer), 0, &triplet.asmErrno);
                triplet.asmElapsedNs = monotonic_ns() - asmStart;
                return triplet;
            },
            &statxItem);
        ...
        const bool memfdSupported = run_syscall_item(
            "memfd_create",
            [&](int attempt) {
                ...
                const long long libcStart = monotonic_ns();
                triplet.libcRet =
                    call_via_syscall(__NR_memfd_create, reinterpret_cast<long>(name.c_str()),
                                     MFD_CLOEXEC, 0, 0, 0, 0, &triplet.libcErrno);
                triplet.libcElapsedNs = monotonic_ns() - libcStart;
                if (triplet.libcRet >= 0)
                    close(static_cast<int>(triplet.libcRet));
                const long long rawStart = monotonic_ns();
                triplet.rawRet =
                    call_via_syscall(__NR_memfd_create, reinterpret_cast<long>(name.c_str()),
                                     MFD_CLOEXEC, 0, 0, 0, 0, &triplet.rawErrno);
                triplet.rawElapsedNs = monotonic_ns() - rawStart;
                if (triplet.rawRet >= 0)
                    close(static_cast<int>(triplet.rawRet));
                const long long asmStart = monotonic_ns();
                triplet.asmRet =
                    asm_syscall6(__NR_memfd_create, reinterpret_cast<long>(name.c_str()),
                                 MFD_CLOEXEC, 0, 0, 0, 0, &triplet.asmErrno);
                triplet.asmElapsedNs = monotonic_ns() - asmStart;
                if (triplet.asmRet >= 0)
                    close(static_cast<int>(triplet.asmRet));
                return triplet;
            },
            &memfdItem);

        SyscallItemAccumulator pidfdItem;
        const bool pidfdSupported = run_syscall_item(
            "pidfd_open",
            [&](int) {
                SyscallAttemptTriplet triplet;
                const pid_t pid = getpid();
                const long long libcStart = monotonic_ns();
                triplet.libcRet =
                    call_via_syscall(__NR_pidfd_open, pid, 0, 0, 0, 0, 0, &triplet.libcErrno);
                triplet.libcElapsedNs = monotonic_ns() - libcStart;
                if (triplet.libcRet >= 0)
                    close(static_cast<int>(triplet.libcRet));
                const long long rawStart = monotonic_ns();
                triplet.rawRet =
                    call_via_syscall(__NR_pidfd_open, pid, 0, 0, 0, 0, 0, &triplet.rawErrno);
                triplet.rawElapsedNs = monotonic_ns() - rawStart;
                if (triplet.rawRet >= 0)
                    close(static_cast<int>(triplet.rawRet));
                const long long asmStart = monotonic_ns();
                triplet.asmRet =
                    asm_syscall6(__NR_pidfd_open, pid, 0, 0, 0, 0, 0, &triplet.asmErrno);
                triplet.asmElapsedNs = monotonic_ns() - asmStart;
                if (triplet.asmRet >= 0)
                    close(static_cast<int>(triplet.asmRet));
                return triplet;
        ...
        const bool renameat2Supported = run_syscall_item(
            "renameat2",
            [&](int attempt) {
                const std::string oldPath =
                    "/proc/self/virtualization_renameat2_old_" + std::to_string(attempt);
                const std::string newPath =
                    "/proc/self/virtualization_renameat2_new_" + std::to_string(attempt);
                SyscallAttemptTriplet triplet;
                const long long libcStart = monotonic_ns();
                triplet.libcRet = call_via_syscall(
                    __NR_renameat2, AT_FDCWD, reinterpret_cast<long>(oldPath.c_str()), AT_FDCWD,
                    reinterpret_cast<long>(newPath.c_str()), 0, 0, &triplet.libcErrno);
                triplet.libcElapsedNs = monotonic_ns() - libcStart;
                const long long rawStart = monotonic_ns();
                triplet.rawRet = call_via_syscall(
                    __NR_renameat2, AT_FDCWD, reinterpret_cast<long>(oldPath.c_str()), AT_FDCWD,
                    reinterpret_cast<long>(newPath.c_str()), 0, 0, &triplet.rawErrno);
                triplet.rawElapsedNs = monotonic_ns() - rawStart;
                const long long asmStart = monotonic_ns();
                triplet.asmRet = asm_syscall6(
                    __NR_renameat2, AT_FDCWD, reinterpret_cast<long>(oldPath.c_str()), AT_FDCWD,
                    reinterpret_cast<long>(newPath.c_str()), 0, 0, &triplet.asmErrno);
                triplet.asmElapsedNs = monotonic_ns() - asmStart;
                return triplet;
            },
            &renameat2Item);

        if (!(openat2Supported && statxSupported && memfdSupported && pidfdSupported &&
              renameat2Supported)) {
    ...
    if (WIFSIGNALED(status) && WTERMSIG(status) == SIGSYS) {
        ...
        return encode_basic_pack(true, false, true,
                                 "Sacrificial syscall child died with SIGSYS. The helper process "
                                 "will not run this pack again.");
    }
...
std::string encode_trap(const TrapResult& result) {
    ...
    for (const auto& attempt : result.attempts) {
        output << "ATTEMPT=" << (attempt.suspicious ? 1 : 0) << '\t' << encode_value(attempt.detail)
               << '\n';
    }
    return output.str();
}
