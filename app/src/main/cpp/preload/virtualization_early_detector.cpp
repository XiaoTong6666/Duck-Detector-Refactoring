...
EarlyVirtualizationResult run_early_detection() {
    ...
    for (const auto& finding : snapshot.findings) {
        result.findings.push_back(finding.label + "|" + finding.value + "|" + finding.severity);
    ...
    if (result.qemuPropertyDetected)
        append_signal(result, "QEMU property");
    if (result.emulatorHardwareDetected)
        append_signal(result, "Emulator hardware");
    if (result.deviceNodeDetected)
        append_signal(result, "Device node");
    if (result.avfRuntimeDetected)
        append_signal(result, "AVF runtime");
    if (result.authfsRuntimeDetected)
        append_signal(result, "authfs runtime");
    if (result.nativeBridgeDetected)
        append_signal(result, "Native bridge");

    result.detected = result.qemuPropertyDetected || result.emulatorHardwareDetected ||
                      result.deviceNodeDetected || result.avfRuntimeDetected ||
                      result.authfsRuntimeDetected || result.nativeBridgeDetected;
    if (result.detected) {
