...
void put_long_extra(JNIEnv* env, jobject intent, jclass intentClass, const char* key,
                    std::int64_t value) {
    jmethodID putLongExtra =
        env->GetMethodID(intentClass, "putExtra", "(Ljava/lang/String;J)Landroid/content/Intent;");
...
void attach_preload_extras(JNIEnv* env, jobject intent, jclass intentClass,
    ...
    put_string_extra(env, intent, intentClass, "early_detection_method", result.detectionMethod);
    put_string_extra(env, intent, intentClass, "early_detection_details", result.details);
    put_boolean_extra(env, intent, intentClass, "early_preload_context_valid",
    ...
    put_boolean_extra(env, intent, intentClass, "early_mount_id_gap", result.mountIdGapDetected);
    put_boolean_extra(env, intent, intentClass, "early_minor_dev_gap", result.minorDevGapDetected);
    ...
    put_string_extra(env, intent, intentClass, "early_mnt_strings_source", result.mntStringsSource);
    put_string_extra(env, intent, intentClass, "early_mnt_strings_target", result.mntStringsTarget);
...
void attach_virtualization_preload_extras(
    JNIEnv* env, jobject intent, jclass intentClass,
    ...
    put_boolean_extra(env, intent, intentClass, "early_virtualization_detected", result.detected);
    ...
    put_boolean_extra(env, intent, intentClass, "early_virtualization_context_valid",
...
void start_main_activity_and_finish(JNIEnv* env, jobject activity) {
    ...
    const duckdetector::preload::virtualization::EarlyVirtualizationResult
        emptyVirtualizationResult{};
    const auto& virtualizationResult =
        virtualizationStored != nullptr ? *virtualizationStored : emptyVirtualizationResult;
    ...
    jmethodID getPackageName =
        env->GetMethodID(activityClass, "getPackageName", "()Ljava/lang/String;");
    ...
    jmethodID setComponent = env->GetMethodID(
        intentClass, "setComponent", "(Landroid/content/ComponentName;)Landroid/content/Intent;");
    jmethodID componentInit =
        env->GetMethodID(componentNameClass, "<init>", "(Ljava/lang/String;Ljava/lang/String;)V");
    ...
    jobject component =
        env->NewObject(componentNameClass, componentInit, packageString, classString);
...
extern "C" __attribute__((visibility("default"))) void native_activity_preload(JNIEnv* env,
                                                                               jobject activity) {
...
Java_com_eltavine_duckdetector_core_startup_preload_EarlyMountPreloadBridge_nativeGetFindings(
    ...
    jobjectArray array =
        env->NewObjectArray(static_cast<jsize>(result->findings.size()), stringClass, nullptr);
...
Java_com_eltavine_duckdetector_core_startup_preload_EarlyMountPreloadBridge_nativeReset(JNIEnv*,
                                                                                        jobject) {
