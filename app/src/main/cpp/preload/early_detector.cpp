...
bool is_platform_tmpfs_namespace_target(const std::string& target) {
    static const char* const kNamespaces[] = {
        "/dev", "/mnt", "/storage",     "/apex",          "/linkerconfig",
        "/sys", "/tmp", "/data_mirror", "/debug_ramdisk",
    };
...
bool find_mount_minor_for_target(const std::vector<MountInfoEntry>& mounts,
                                 const std::string& target, unsigned int& outMinor) {
...
bool summarize_data_mirror_mount_id_gap(const std::vector<MountInfoEntry>& mounts,
                                        std::size_t dataMirrorIndex, MountIdGapSummary& summary) {
...
bool detect_futile_hide(EarlyMountPreloadResult& result) {
    ...
    if (fstatat(AT_FDCWD, "/proc/self/ns/mnt", &nsStat, AT_SYMLINK_NOFOLLOW) == 0) {
        ...
        if (delta < -kSecToNs) {
            ...
            result.findings.push_back(
                "FUTILE_HIDE|ns/mnt ctime anomaly: delta=" + std::to_string(delta) + " ns|DANGER");
        }
    }
    ...
    if (stat("/proc/self/mountinfo", &mountInfoStat) == 0) {
        const std::int64_t delta =
            ((static_cast<std::int64_t>(mountInfoStat.st_ctim.tv_sec) - now.tv_sec) * kSecToNs) +
            (mountInfoStat.st_ctim.tv_nsec - now.tv_nsec);
        result.mountInfoCtimeDeltaNs = delta;
        if (delta < -kSecToNs) {
            detected = true;
            result.futileHideDetected = true;
            result.findings.push_back("FUTILE_HIDE|mountinfo ctime anomaly: delta=" +
                                      std::to_string(delta) + " ns|DANGER");
        }
    }
...
bool detect_mnt_strings_anomaly(EarlyMountPreloadResult& result) {
    ...
    if (source == "KSU" || source == "magisk" || source == "APatch") {
        ...
        result.findings.push_back("MNT_STRINGS|Suspicious source name in mntent: " + source +
                                  "|DANGER");
    }
    ...
    if (target.starts_with("/data/adb")) {
        detected = true;
        result.mntStringsDetected = true;
        result.findings.push_back("MNT_STRINGS|Suspicious target path in mntent: " + target +
                                  "|DANGER");
    }
    ...
    for (std::size_t index = 0; index < mounts.size(); ++index) {
        ...
        if (mount.target == "/apex/com.android.art" && index + 1 < mounts.size()) {
            ...
            if (next.id > mount.id + 1) {
                ...
                result.findings.push_back(
                    ...
                    std::to_string(mount.id + 1) + ", got " + std::to_string(next.id) + "|DANGER");
            }
        }
        ...
        if (mount.target == "/data_mirror") {
            ...
            if (summarize_data_mirror_mount_id_gap(mounts, index, summary)) {
                detected = true;
                result.mountIdGapDetected = true;
                result.findings.push_back("MOUNT_ID_GAP|Missing " +
                                          std::to_string(summary.missingCount) +
...
bool detect_minor_dev_gap(EarlyMountPreloadResult& result) {
    ...
    if (!find_mount_minor_for_target(mounts, "/dev", devMinor) ||
        !find_mount_minor_for_target(mounts, "/sys", sysMinor) || sysMinor <= devMinor) {
        return false;
    }
    ...
    std::sort(tmpfsMounts.begin(), tmpfsMounts.end(),
    ...
    for (const auto& mount : tmpfsMounts) {
        ...
        if (mount.minor != lastMinor + 1) {
            result.findings.push_back(
                "MINOR_DEV_GAP|Expected minor " + std::to_string(lastMinor + 1) + ", got " +
                std::to_string(mount.minor) + " at " + mount.target + "|WARNING");
        } else {
            result.findings.push_back(
                "MINOR_DEV_GAP|Unexpected tmpfs target before /sys: " + mount.target + " (minor " +
                std::to_string(mount.minor) + ")|WARNING");
        }
        return true;
    }
...
bool detect_peer_group_gap(EarlyMountPreloadResult& result) {
    ...
    if (gapCount > kAllowedGaps) {
        ...
        result.findings.push_back("PEER_GROUP_GAP|Found " + std::to_string(gapCount) +
                                  " gaps (allowed " + std::to_string(kAllowedGaps) + ")|WARNING");
        return true;
    }
...
EarlyMountPreloadResult run_early_detection() {
    ...
    result.details = detected ? "Early mount preload detected: " + result.detectionMethod
