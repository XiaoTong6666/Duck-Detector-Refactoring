...
constexpr Finding kOverlayPatterns[] = {
    {"LineageOS", "lineage"},     {"crDroid", "crdroid"},
    {"ParanoidAndroid", "aospa"}, {"PixelExperience", "pixelexperience"},
    {"Evolution-X", "evolution"}, {"OmniROM", "omnirom"},
...
std::vector<std::string> parse_map_injection() {
    ...
    while (std::getline(stream, line)) {
        ...
        const std::string device_id =
            line.substr(dev_start_pos + 1, dev_end_pos - dev_start_pos - 1);
    ...
    if (device_mappings.size() >= 3 || has_custom_rom_path) {
        findings.emplace_back("framework-res.apk mapped from " +
                              std::to_string(device_mappings.size()) +
                              " devices (possible overlay injection)");
        for (const auto& [device_id, info] : device_mappings) {
            findings.emplace_back("Device " + device_id + ": " + std::to_string(info.count) +
                                  " mappings");
        }
    }
...
Java_com_eltavine_duckdetector_features_customrom_data_native_CustomRomNativeBridge_nativeCollectSnapshot(
    ...
    for (const auto& finding : property_integrity.findings) {
        output << "MODIFICATION=" << escape_line(finding.category) << "|"
               << escape_line(finding.signal) << "|" << escape_line(finding.summary) << "|"
    ...
    if (symbol_scan_available) {
        for (const auto& path :
             customrom::find_loaded_symbol_paths("libstagefright.so", kSymbolStagefright)) {
