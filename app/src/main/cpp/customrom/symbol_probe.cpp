...
bool contains_symbol_in_file(const std::string& path, const char* symbol_name) {
    ...
    const bool valid =
        st.st_size >= static_cast<off_t>(sizeof(ElfHeader)) &&
        std::memcmp(ehdr->e_ident, ELFMAG, SELFMAG) == 0 && ehdr->e_shoff != 0 &&
        ehdr->e_shentsize == sizeof(ElfSection) && ehdr->e_shnum > 0 &&
    ...
    for (uint16_t index = 0; index < ehdr->e_shnum; ++index) {
        ...
        const auto* symbols = reinterpret_cast<const ElfSymbol*>(base + section.sh_offset);
        ...
        for (size_t symbol_index = 0; symbol_index < symbol_count; ++symbol_index) {
            ...
            if (symbol.st_name >= strtab.sh_size || symbol.st_shndx == SHN_UNDEF ||
                symbol.st_value == 0) {
                continue;
            }

            const char* name =
                reinterpret_cast<const char*>(base + strtab.sh_offset + symbol.st_name);
