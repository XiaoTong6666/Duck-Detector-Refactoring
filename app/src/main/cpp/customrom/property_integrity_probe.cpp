...
bool is_skipped_entry(const char* name) {
    ...
    return std::strcmp(name, ".") == 0 || std::strcmp(name, "..") == 0 || kSerialFilename == name ||
...
std::optional<PropertyIntegrityFinding> build_area_finding(
    const std::string& context, const std::vector<std::string>& problems) {
    if (problems.empty()) {
        return std::nullopt;
    }
...
std::optional<PropertyIntegrityFinding> check_area_permissions(const std::string& path,
    ...
    if (mode != 0444 || st.st_uid != 0 || st.st_gid != 0) {
        std::ostringstream stream;
        stream << "mode=" << std::oct << mode << std::dec << " uid=" << st.st_uid
               << " gid=" << st.st_gid;
        problems.push_back(stream.str());
    }

    if (const auto label = read_selinux_context(path); label.has_value() && *label != context) {
...
class PropAreaParser {
   public:
    ...
    bool parse(std::vector<HoleRun>* holes, std::vector<ParsedProperty>* properties) {
        ...
        if (mark_dirty_backup_ &&
            !mark_range(static_cast<uint32_t>(sizeof(DiskPropTrieNode)), kDirtyBackupAreaSize)) {
            return false;
        }
   ...
   private:
    ...
    bool visit_root(const DiskPropTrieNode* root) {
        return visit_trie(root->left) && visit_trie(root->right) && visit_trie(root->children) &&
    ...
    bool visit_trie(uint32_t offset) {
        ...
        const uint32_t name_offset = offset + static_cast<uint32_t>(sizeof(DiskPropTrieNode));
        const auto name_length = bounded_c_string_length(name_offset);
        if (!name_length.has_value() || *name_length != node->namelen) {
            return false;
        }

        const size_t allocation_size = align_up(sizeof(DiskPropTrieNode) + node->namelen + 1U);
        if (!mark_range(offset, allocation_size)) {
            return false;
        }

        return visit_trie(node->left) && visit_trie(node->right) && visit_trie(node->children) &&
    ...
    bool visit_prop(uint32_t offset) {
        ...
        const std::string property_name(reinterpret_cast<const char*>(data_ + name_offset),
                                        *name_length);
                ...
                if (relative_offset >= sizeof(DiskPropInfo)) {
                    ...
                    if (value_length.has_value()) {
                        const auto copy_length =
                            std::min(*value_length, static_cast<size_t>(PROP_VALUE_MAX - 1));
                        std::memcpy(entry.value.data(), data_ + long_value_offset, copy_length);
    ...
    bool mark_range(uint32_t offset, size_t size) {
        ...
        std::fill(occupied_.begin() + static_cast<std::ptrdiff_t>(offset),
                  occupied_.begin() + static_cast<std::ptrdiff_t>(offset + size), 1);
        return true;
    }
...
std::optional<AreaScanResult> scan_area(const std::string& path, const std::string& context,
                                        bool mark_dirty_backup) {
    ...
    void* mapping =
        mmap(nullptr, static_cast<size_t>(stat_buffer.st_size), PROT_READ, MAP_PRIVATE, fd, 0);
    close(fd);
    ...
    const size_t data_size = static_cast<size_t>(stat_buffer.st_size) - sizeof(DiskPropAreaHeader);
    const size_t minimum_bytes_used =
        sizeof(DiskPropTrieNode) + (mark_dirty_backup ? kDirtyBackupAreaSize : 0);
    if (header->magic != kPropAreaMagic || header->version != kPropAreaVersion ||
        header->bytes_used > data_size || header->bytes_used < minimum_bytes_used) {
        munmap(mapping, static_cast<size_t>(stat_buffer.st_size));
        return std::nullopt;
    }

    AreaScanResult result;
    PropAreaParser parser(header->data, header->bytes_used, context, mark_dirty_backup);
...
void add_area_anomaly(PropertyIntegritySnapshot& snapshot, const std::string& signal,
...
void add_property_anomaly(PropertyIntegritySnapshot& snapshot, const std::string& category,
                          const std::string& signal, const std::string& summary,
                          const std::string& detail) {
    snapshot.findings.push_back(PropertyIntegrityFinding{
        .category = category,
        .signal = signal,
...
void inspect_property(PropertyIntegritySnapshot& snapshot, const ParsedProperty& property) {
    if ((property.serial & kLongFlag) == 0 && (property.serial & kSerialResidueMask) != 0) {
        add_property_anomaly(snapshot, "Prop serial", property.name, "Abnormal prop serial",
    ...
    if (!property.is_long && starts_with_ro(property.name) && has_tail_bytes(property.value)) {
        add_property_anomaly(snapshot, "Prop tail", property.name, "Residual prop value",
