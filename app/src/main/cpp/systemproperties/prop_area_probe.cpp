...
class PropAreaParser {
   public:
    PropAreaParser(const uint8_t* data, size_t bytes_used)
        : data_(data), bytes_used_(bytes_used), occupied_(bytes_used, 0) {
    }
    ...
    bool parse(std::vector<HoleRun>* holes) {
        ...
        if (!mark_range(static_cast<uint32_t>(sizeof(DiskPropTrieNode)), kDirtyBackupAreaSize)) {
            return false;
        }
...
std::optional<ParsedAreaResult> scan_area(const std::string& path, const std::string& context) {
        ...
        if (parser.parse(&holes)) {
            ...
            if (!holes.empty()) {
                ...
                for (size_t index = 0; index < sample_count; ++index) {
                    ...
                    detail << '@' << hex_offset(holes[index].offset) << '+' << holes[index].length;
                }
