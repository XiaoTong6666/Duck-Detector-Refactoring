...
void append_probe_findings(Snapshot& snapshot, const ProbeResult& probe,
...
Snapshot collect_snapshot(const bool skip_ksu_supercall) {
    ...
    const ProbeResult kernelpatch_supercall_latency_probe =
        run_kernelpatch_supercall_latency_check();
    ...
    snapshot.kernel_su_version = prctl_probe.numeric_value > 0 ? prctl_probe.numeric_value
    ...
    snapshot.devpts_abnormal_permission_detected = devpts_abnormal_permission_probe.flags.root ||
                                                   devpts_abnormal_permission_probe.flags.kernel_su;
    ...
    snapshot.devpts_abnormal_permission_checked_count =
        devpts_abnormal_permission_probe.checked_count;
    snapshot.devpts_abnormal_permission_denied_count =
        devpts_abnormal_permission_probe.denied_count;
    ...
    snapshot.ksu_supercall_lkm = (ksu_supercall_probe.extra_numeric_value & (1U << 0)) != 0;
    snapshot.ksu_supercall_manager = (ksu_supercall_probe.extra_numeric_value & (1U << 1)) != 0;
    snapshot.ksu_supercall_late_load = (ksu_supercall_probe.extra_numeric_value & (1U << 2)) != 0;
    snapshot.ksu_supercall_pr_build = (ksu_supercall_probe.extra_numeric_value & (1U << 3)) != 0;
    ...
    snapshot.self_ksu_fdwrapper_count =
        static_cast<int>(self_process_ioc_probe.extra_numeric_value);
