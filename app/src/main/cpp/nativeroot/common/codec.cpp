...
std::string encode_snapshot(const Snapshot& snapshot) {
    ...
    output << "KERNELPATCH_SIDE_CHANNEL_ATTACK="
           << (snapshot.kernelpatch_side_channel_detected ? '1' : '0') << '\n';
    output << "KERNELPATCH_SIDE_CHANNEL_DETAIL="
           << escape_value(snapshot.kernelpatch_side_channel_detail) << '\n';
    output << "DEVPTS_ABNORMAL_PERMISSION_FOUND="
           << (snapshot.devpts_abnormal_permission_detected ? '1' : '0') << '\n';
    output << "DEVPTS_ABNORMAL_PERMISSION_AVAILABLE="
           << (snapshot.devpts_abnormal_permission_available ? '1' : '0') << '\n';
    output << "DEVPTS_ABNORMAL_PERMISSION_CHECKED="
           << snapshot.devpts_abnormal_permission_checked_count << '\n';
    output << "DEVPTS_ABNORMAL_PERMISSION_DENIED="
           << snapshot.devpts_abnormal_permission_denied_count << '\n';
    output << "DEVPTS_ABNORMAL_PERMISSION_DETAIL="
           << escape_value(snapshot.devpts_abnormal_permission_detail) << '\n';
    ...
    for (const Finding& finding : snapshot.findings) {
        output << "FINDING=" << finding.group << '\t' << to_string(finding.severity) << '\t'
               << escape_value(finding.label) << '\t' << escape_value(finding.value) << '\t'
