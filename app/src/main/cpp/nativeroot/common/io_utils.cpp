...
bool directory_contains_entry(const char* directory_path, const char* entry_name) {
    ...
    const int fd = syscall_openat_readonly(directory_path, O_RDONLY | O_DIRECTORY | O_CLOEXEC);
    if (fd < 0) {
        return false;
    }
...
bool contains_ignore_case(const std::string& haystack, const char* needle) {
    return lowercase_copy(haystack).find(lowercase_copy(std::string(needle))) != std::string::npos;
}

bool contains_any_ignore_case(const std::string& haystack,
