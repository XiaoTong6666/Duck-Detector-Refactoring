...
Java_com_eltavine_duckdetector_features_nativeroot_data_native_CgroupProcessLeakNativeBridge_nativeCollectSnapshot(
    ...
    for (const auto& path : snapshot.paths) {
        output << "PATH=" << escape_value(path.path) << '\t' << path.uid << '\t'
               << (path.accessible ? '1' : '0') << '\t' << path.pid_count << '\n';
    }
    for (const auto& entry : snapshot.entries) {
        output << "ENTRY=" << escape_value(entry.uid_path) << '\t' << entry.cgroup_uid << '\t'
               << entry.pid << '\t' << entry.proc_uid << '\t' << entry.starttime_ticks << '\t'
               << entry.kill_errno << '\t' << entry.getsid_value << '\t' << entry.getsid_errno
               << '\t' << entry.getpgid_value << '\t' << entry.getpgid_errno << '\t'
               << entry.sched_policy << '\t' << entry.sched_errno << '\t' << entry.pidfd_errno
               << '\t' << escape_value(entry.proc_context) << '\t' << escape_value(entry.comm)
