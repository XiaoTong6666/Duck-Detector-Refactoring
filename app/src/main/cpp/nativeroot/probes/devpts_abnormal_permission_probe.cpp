...
void note_error(ProbeResult& result, const char* path, const char* step, const int error) {
    result.denied_count++;
...
void inspect_pty_path(const char* path, ProbeResult& result, const bool required_sample) {
    struct stat st{};
    if (lstat(path, &st) == -1) {
        if (required_sample) {
    ...
    if (len >= 0) {
        attr_val[len] = '\0';
        append_line(finding.detail, std::string("SELinux: ") + (len > 0 ? attr_val : "<empty>"));
