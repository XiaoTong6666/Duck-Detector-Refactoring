...
Finding build_prctl_finding(const std::string& value, const std::string& detail,
...
ProbeResult run_prctl_probe() {
    ...
    if (version > 0) {
        ...
        result.findings.push_back(build_prctl_finding(
            "v" + std::to_string(version),
            "prctl(0xDEADBEEF, 2) returned KernelSU version " + std::to_string(version) + ".",
            Severity::kDanger));
        return result;
    }
    ...
    for (const int command : kAlternativeCommands) {
        ...
        if (response >= 0 && errno == 0) {
            ...
            std::snprintf(detail, sizeof(detail),
                          ...
                          command, response);
            result.findings.push_back(build_prctl_finding(
                command == kKernelSuVersionCommand && response > 0 ? "v" + std::to_string(response)
                                                                   : "Response",
                detail, Severity::kDanger));
            break;
        }
    }
