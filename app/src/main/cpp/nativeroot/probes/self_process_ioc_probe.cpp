...
ProbeResult run_self_process_ioc_probe() {
    ...
    const int proc_fd = syscall_openat_readonly(kSelfFdPath, O_RDONLY | O_DIRECTORY | O_CLOEXEC);
    if (proc_fd >= 0) {
    ...
    if (driver_fd_count > 0 || fdwrapper_count > 0) {
        ...
        result.findings.push_back(Finding{
            ...
            .detail = "The current app process already holds KernelSU [ksu_driver] or "
                      "[ksu_fdwrapper] descriptors before privileged escalation.",
            .severity = Severity::kDanger,
        });
    }
