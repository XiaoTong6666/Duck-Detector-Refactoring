...
constexpr const char* kUidRootPaths[] = {
    ...
    "/dev/cg2_bpf",   "/dev/cg2_bpf/apps",   "/dev/cg2_bpf/system",
    "/acct",          "/dev/memcg/apps",
};

constexpr const char* kFallbackUidPathPatterns[] = {
    "/sys/fs/cgroup/uid_%d", "/sys/fs/cgroup/apps/uid_%d", "/sys/fs/cgroup/system/uid_%d",
    "/dev/cg2_bpf/uid_%d",   "/dev/cg2_bpf/apps/uid_%d",   "/dev/cg2_bpf/system/uid_%d",
    "/acct/uid_%d",          "/dev/memcg/apps/uid_%d",
};
...
int parse_pid_dir(const char* name, const size_t max_length) {
    ...
    if (!is_numeric_name(pid_chars,
                         max_length >= sizeof(kPrefix) ? max_length - (sizeof(kPrefix) - 1) : 0)) {
        return -1;
    }
...
void append_uid_path(const std::string& path, int uid,
...
void collect_uid_paths_from_root(const char* root_path,
                                 std::vector<std::pair<std::string, int>>& paths,
                                 std::set<std::string>& dedupe) {
    ...
    while (true) {
        ...
        while (offset < bytes_read) {
            ...
            if (uid >= 0) {
                append_uid_path(std::string(root_path) + "/" + std::string(entry->d_name), uid,
                                paths, dedupe);
            }
...
CgroupLeakSnapshot collect_cgroup_leak_snapshot() {
    ...
    for (const auto& [uid_path, uid] : uid_paths) {
        ...
        const int uid_fd =
            syscall_openat_readonly(uid_path.c_str(), O_RDONLY | O_DIRECTORY | O_CLOEXEC);
        if (uid_fd < 0) {
        ...
        char dent_buffer[4096];
        while (true) {
            const int bytes_read = syscall_getdents64_fd(uid_fd, dent_buffer, sizeof(dent_buffer));
            if (bytes_read <= 0) {
                break;
            }
            ...
            while (offset < bytes_read) {
                ...
                if (pid > 0) {
                    ...
                    if (dedupe.insert(dedupe_key).second) {
                        snapshot.entries.push_back(CgroupLeakProcessEntry{
                            .uid_path = uid_path,
                            .cgroup_uid = uid,
                            .pid = pid,
                            ...
                            .sched_policy =
                                sched_result.value >= 0 ? static_cast<int>(sched_result.value) : -1,
