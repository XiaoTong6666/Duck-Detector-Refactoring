...
constexpr const char* kKernelTokens[] = {
    "ksu_", "kernelsu", "apatch", "kpatch", "supercall", "magisk",
};

void apply_token_flags(const std::string& token, DetectionFlags& flags) {
    if (token.find("ksu") != std::string::npos || token.find("kernelsu") != std::string::npos) {
        flags.kernel_su = true;
    }
    if (token.find("apatch") != std::string::npos || token.find("kpatch") != std::string::npos ||
...
void append_source_finding(ProbeResult& result, const char* label, const char* detail_prefix,
