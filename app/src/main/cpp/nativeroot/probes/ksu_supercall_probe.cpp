...
bool collect_child_packet(KsuSupercallPacket& packet, bool& blocked_by_seccomp) {
    int pipe_fds[2] = {-1, -1};
    if (pipe(pipe_fds) != 0) {
        return false;
    }
    ...
    if (pid == 0) {
        ...
        int driver_fd = -1;
        syscall(__NR_reboot, kKsuInstallMagic1, kKsuInstallMagic2, 0, &driver_fd);

        if (driver_fd >= 0) {
            KsuGetInfoCmd info_cmd{};
            if (ioctl(driver_fd, kKsuIoctlGetInfo, &info_cmd) == 0 && info_cmd.version != 0) {
...
Finding build_finding(const std::string& value, const std::string& detail,
...
ProbeResult run_ksu_supercall_probe() {
    ...
    std::string detail =
        "A sacrificial child installed temporary [ksu_driver] and KSU_IOCTL_GET_INFO returned "
        "version " +
    ...
    result.findings.push_back(
        build_finding("v" + std::to_string(packet.version), detail, Severity::kDanger));
