...
std::atomic<int> g_statxAvailability{static_cast<int>(StatxAvailability::Unknown)};
#endif
...
bool is_statx_supported() {
    const auto cached =
        static_cast<StatxAvailability>(g_statxAvailability.load(std::memory_order_acquire));
    ...
    if (pid < 0) {
        ...
        errno = 0;
        const long result =
            syscall(__NR_statx, AT_FDCWD, "/", AT_NO_AUTOMOUNT, STATX_BASIC_STATS, &stx);
        if (result == 0) {
            _exit(0);
        }
        ...
        if (waitpid(pid, &status, 0) < 0) {
            ...
            LOGI("statx support probe reported unsupported or inconclusive status=%d", status);
        }
    }

    g_statxAvailability.store(static_cast<int>(resolved), std::memory_order_release);
...
AccessResult check_access(const char* path, bool expectDirectory, MountSnapshot& snapshot) {
...
std::vector<MountEntry> parse_mounts(const std::string& raw) {
    ...
    while (std::getline(stream, line)) {
        ...
        if (!(lineStream >> entry.source >> entry.target >> entry.fsType >> entry.options)) {
            continue;
        }
...
std::vector<MountInfoEntry> parse_mount_info(const std::string& raw) {
    ...
    std::sort(
        ...
        [](const MountInfoEntry& left, const MountInfoEntry& right) { return left.id < right.id; });
    return entries;
}

void add_finding(MountSnapshot& snapshot, std::unordered_set<std::string>& dedupe,
                 const std::string& group, const std::string& severity, const std::string& label,
                 const std::string& value, const std::string& detail) {
    const std::string key = group + "|" + label + "|" + value + "|" + detail;
    if (!dedupe.insert(key).second) {
        return;
    }
    snapshot.findings.push_back(MountFindingRecord{
        group,
        severity,
...
void detect_busybox(MountSnapshot& snapshot, std::unordered_set<std::string>& dedupe) {
    static const std::array<const char*, 11> kBusyboxPaths = {
        "/system/xbin/busybox",      "/system/bin/busybox",      "/system/sd/xbin/busybox",
        "/vendor/bin/busybox",       "/product/bin/busybox",     "/sbin/busybox",
        "/data/local/xbin/busybox",  "/data/local/bin/busybox",  "/data/adb/magisk/busybox",
    ...
    for (const char* path : kBusyboxPaths) {
        ...
        if (access == AccessResult::Exists) {
            ...
            add_finding(snapshot, dedupe, "ARTIFACTS", "WARNING", "Busybox binary", "Present",
                        path);
        }
    }
}

void detect_root_data_paths(MountSnapshot& snapshot, std::unordered_set<std::string>& dedupe) {
    const AccessResult dataAdbAccess = check_access("/data/adb", true, snapshot);
    if (dataAdbAccess == AccessResult::PermissionDenied) {
        add_finding(snapshot, dedupe, "CONSISTENCY", "INFO", "/data/adb access", "Restricted",
    ...
    static const std::array<RootPathSpec, 9> kRootPaths = {{
        {"/data/adb/magisk", "Magisk data directory", true},
        {"/data/adb/magisk.db", "Magisk database", false},
        {"/data/adb/ksu", "KernelSU data directory", true},
        {"/data/adb/ksud", "KernelSU daemon", false},
        {"/data/adb/ap", "APatch data directory", true},
        {"/data/adb/apd", "APatch daemon", false},
        {"/data/adb/modules", "Root modules directory", true},
        {"/data/adb/service.d", "Root service scripts", true},
        {"/data/adb/post-fs-data.d", "Post-fs-data scripts", true},
    }};
    ...
    for (const RootPathSpec& spec : kRootPaths) {
        if (check_access(spec.path, spec.isDirectory, snapshot) == AccessResult::Exists) {
            ...
            add_finding(snapshot, dedupe, "ARTIFACTS", "DANGER", spec.label, "Present", spec.path);
        }
    }
}

void detect_debug_ramdisk(MountSnapshot& snapshot, std::unordered_set<std::string>& dedupe) {
    struct DebugSpec {
        const char* path;
        const char* label;
        bool isDirectory;
    ...
    static const std::array<DebugSpec, 4> kSpecs = {{
        {"/debug_ramdisk/adb_debug.prop", "adb_debug.prop", false, "DANGER"},
        {"/debug_ramdisk/userdebug_plat_sepolicy.cil", "userdebug sepolicy", false, "DANGER"},
        {"/debug_ramdisk/force_debuggable", "force_debuggable", false, "DANGER"},
        {"/debug_ramdisk/force_adb", "force_adb", false, "DANGER"},
    }};

    for (const DebugSpec& spec : kSpecs) {
        if (check_access(spec.path, spec.isDirectory, snapshot) == AccessResult::Exists) {
            snapshot.debugRamdiskDetected = true;
            add_finding(snapshot, dedupe, "ARTIFACTS", spec.severity, spec.label, "Present",
                        spec.path);
        }
    }
}

void detect_hybrid_paths(MountSnapshot& snapshot, std::unordered_set<std::string>& dedupe) {
    struct HybridSpec {
        const char* path;
        const char* label;
        bool isDirectory;
    ...
    static const std::array<HybridSpec, 4> kSpecs = {{
        {"/dev/meta_hybird_mnt", "Meta hybrid device", false, false},
        {"/patch_hw", "Meta-Hybrid patch path", true, true},
        {"/.magic_mount", "Magic mount workdir", true, true},
        {"/data/adb/modules/.core", "Root core modules", true, true},
    }};
...
void detect_maps(MountSnapshot& snapshot, const std::string& mapsRaw,
    ...
    static const std::array<std::pair<const char*, const char*>, 5> kPatterns = {{
        {"libzygisk.so", "Zygisk library"},
        {"libriru.so", "Riru library"},
        {"libriru_", "Riru module library"},
        {"/.magisk/", "Magisk hidden memory path"},
        {"/sbin/.magisk", "Magisk sbin memory path"},
    }};
    ...
    while (std::getline(stream, line)) {
        ...
        for (const auto& pattern : kPatterns) {
            if (line.find(pattern.first) != std::string::npos) {
                ...
                add_finding(snapshot, dedupe, "ARTIFACTS", "DANGER", pattern.second, "Mapped",
                            display);
            }
        }
    }
...
void detect_mounts(MountSnapshot& snapshot, const std::vector<MountEntry>& mounts,
                   std::unordered_set<std::string>& dedupe) {
    ...
    for (const MountEntry& entry : mounts) {
        ...
        if (lowerSource.find(".magisk") != std::string::npos ||
            ...
            add_finding(snapshot, dedupe, "ARTIFACTS", "DANGER", "Magisk mounts", entry.target,
        ...
        if (is_system_partition(entry.target)) {
            ...
            if (isRw) {
                ...
                add_finding(snapshot, dedupe, "RUNTIME", "DANGER", "System RW", entry.target,
            ...
            if ((lowerFs == "overlay" || lowerFs == "overlayfs") &&
                ...
                add_finding(snapshot, dedupe, "RUNTIME", "DANGER", "Overlay mount", entry.target,
                            entry.source + " -> " + entry.target + " (" + entry.fsType + ")");
            }
            ...
            if (lowerSource.find("/dev/block/loop") != std::string::npos ||
                ...
                add_finding(snapshot, dedupe, "RUNTIME", "DANGER", "Loop device mount",
                            entry.target, entry.source + " -> " + entry.target);
            }

            if (lowerSource.find("/dev/mapper/") == std::string::npos &&
                lowerSource.find("/dev/block/dm-") == std::string::npos && lowerFs != "overlay" &&
                lowerFs != "overlayfs") {
                snapshot.dmVerityBypassDetected = true;
                add_finding(snapshot, dedupe, "RUNTIME", "WARNING", "Direct block mount",
                            entry.target, "Expected dm-verity mapper source, got " + entry.source);
            }
        }

        if (lowerFs == "tmpfs" &&
            (lowerTarget == "/sbin" || lowerTarget == "/.magisk" || lowerTarget == "/patch_hw" ||
             lowerTarget == "/dev/meta_hybird_mnt")) {
            snapshot.suspiciousTmpfsDetected = true;
            snapshot.tmpfsSizeAnomaly = true;
            add_finding(snapshot, dedupe, "FILESYSTEM", "WARNING", "Suspicious tmpfs", entry.target,
        ...
        if (lowerSource.find("ksu") != std::string::npos ||
            ...
            add_finding(snapshot, dedupe, "ARTIFACTS", "DANGER", "KernelSU or magic-mount overlay",
                        entry.target, entry.source + " -> " + entry.target);
        }
        ...
        if (lowerSource.find("meta-hybrid") != std::string::npos ||
            ...
            snapshot.metaHybridMountDetected = true;
            add_finding(snapshot, dedupe, "ARTIFACTS", "DANGER", "Meta-Hybrid or magic mount",
                        entry.target, entry.source + " -> " + entry.target);
        }
    }
}

void detect_mountinfo(MountSnapshot& snapshot, const std::vector<MountInfoEntry>& entries,
                      std::unordered_set<std::string>& dedupe) {
    ...
    for (size_t index = 0; index < entries.size(); ++index) {
        ...
        if (is_system_partition(entry.target) && entry.root != "/") {
            ...
            add_finding(snapshot, dedupe, "CONSISTENCY", "DANGER", "Bind mount root", entry.target,
        ...
        if (is_system_partition(entry.target) &&
            ...
            add_finding(snapshot, dedupe, "CONSISTENCY", "DANGER", "SELinux mount override",
                        entry.target, entry.superOptions);
        }
        ...
        if (entry.target == "/" && lowerOptional.find("shared:") == std::string::npos &&
            ...
            add_finding(snapshot, dedupe, "CONSISTENCY", "WARNING", "Root mount propagation",
                        "Private",
        ...
        if (entry.target == "/apex/com.android.art" && index + 1 < entries.size() &&
            ...
            add_finding(snapshot, dedupe, "CONSISTENCY", "DANGER", "Mount ID loophole",
                        "Gap after ART",
        ...
        if (entry.target == "/data_mirror" && index > 0) {
            ...
            if (previous.id + 1 != entry.id &&
                ...
                snapshot.mountIdLoopholeDetected = true;
                add_finding(
                    snapshot, dedupe, "CONSISTENCY", "DANGER", "Mount ID loophole",
                    "Gap before /data_mirror",
                    "Previous mount " + previous.target + " has ID " + std::to_string(previous.id));
            }
        }
    }
    ...
    if (snapshot.statxSupported && dataDataMountId > 0) {
        struct statx stx{};
        const int statxResult =
            static_cast<int>(syscall(__NR_statx, AT_FDCWD, "/data/data", AT_NO_AUTOMOUNT,
        ...
        if (statxResult == 0 && (stx.stx_mask & kStatxMountIdMask) != 0 &&
            ...
            add_finding(
                snapshot, dedupe, "CONSISTENCY", "DANGER", "statx mount ID mismatch", "/data/data",
                "mountinfo=" + std::to_string(dataDataMountId) +
                    ", statx=" + std::to_string(static_cast<unsigned long long>(stx.stx_mnt_id)));
        }
    }
    ...
    if (snapshot.statxSupported) {
        for (const StatxCheck& check : kChecks) {
            struct statx stx{};
            const int statxResult =
                static_cast<int>(syscall(__NR_statx, AT_FDCWD, check.path, AT_NO_AUTOMOUNT,
                                         STATX_BASIC_STATS | kStatxMountIdMask, &stx));
            if (statxResult != 0 || (stx.stx_mask & kStatxMountIdMask) == 0) {
                continue;
            }

            const bool mountRootFlagKnown = (stx.stx_attributes_mask & kStatxMountRootMask) != 0;
            const bool isMountRoot =
                mountRootFlagKnown && (stx.stx_attributes & kStatxMountRootMask) != 0;
            if (isMountRoot) {
                snapshot.statxMountRootAnomaly = true;
                add_finding(
                    snapshot, dedupe, "CONSISTENCY", "WARNING", "statx mount root", check.path,
                    std::string(check.label) + " unexpectedly reports mount-root attributes.");
            }
            ...
            if (mountIdIt != mountIds.end() &&
                ...
                snapshot.statxMountRootAnomaly = true;
                add_finding(snapshot, dedupe, "CONSISTENCY", "DANGER", "statx mount cross-check",
                            check.path,
...
void detect_namespace(MountSnapshot& snapshot, std::unordered_set<std::string>& dedupe) {
    ...
    if (selfNs != initNs) {
        add_finding(snapshot, dedupe, "CONSISTENCY", "INFO", "Namespace split",
                    "Different from init", "self=" + selfNs + ", init=" + initNs);
    } else {
        add_finding(snapshot, dedupe, "CONSISTENCY", "WARNING", "Namespace split", "Matches init",
                    "The app shares the same mount namespace as init, which is unusual for an "
                    "unprivileged app.");
    }
}

void detect_filesystems(MountSnapshot& snapshot, const std::vector<MountEntry>& mounts,
                        std::unordered_set<std::string>& dedupe) {
    bool filesystemsReadable = false;
    const std::string filesystems =
        read_file_direct("/proc/filesystems", 16384, &filesystemsReadable);
    ...
    for (const StatFsCheck& check : kChecks) {
        ...
        if ((path == "/system" || path == "/vendor" || path == "/product" ||
             path == "/system_ext" || path == "/odm") &&
            static_cast<unsigned long>(st.f_type) ==
                static_cast<unsigned long>(OVERLAYFS_SUPER_MAGIC)) {
            snapshot.systemFsTypeAnomaly = true;
            add_finding(snapshot, dedupe, "FILESYSTEM", "DANGER", "System filesystem type",
                        check.path, std::string(check.label) + " resolved to overlayfs.");
        }

        if (static_cast<unsigned long>(st.f_type) == static_cast<unsigned long>(TMPFS_MAGIC) &&
            (path == "/system" || path == "/vendor")) {
            snapshot.tmpfsSizeAnomaly = true;
            add_finding(snapshot, dedupe, "FILESYSTEM", "WARNING", "System tmpfs", check.path,
    ...
    for (const MountEntry& entry : mounts) {
        if (lowercase_copy(entry.fsType) == "tmpfs" && entry.target == "/sbin") {
            snapshot.tmpfsSizeAnomaly = true;
            add_finding(snapshot, dedupe, "FILESYSTEM", "WARNING", "Tmpfs mount", entry.target,
                        entry.options);
        }
    }
}

void detect_inconsistent_mount(MountSnapshot& snapshot, const std::vector<MountEntry>& mounts,
                               std::unordered_set<std::string>& dedupe) {
    std::array<char, PATH_MAX> resolved{};
    const ssize_t resolvedLength =
        syscall_readlink_path("/proc/self/exe", resolved.data(), resolved.size() - 1);
    if (resolvedLength <= 0) {
        return;
    }
    ...
    if (statfs("/proc/self/exe", &directFs) != 0 || statfs(resolved.data(), &resolvedFs) != 0) {
        return;
    }
    ...
    if (directFs.f_type != resolvedFs.f_type) {
        ...
        add_finding(
            snapshot, dedupe, "CONSISTENCY", "DANGER", "Mount consistency", "Filesystem mismatch",
            "direct=0x" + std::to_string(static_cast<unsigned long>(directFs.f_type)) +
                ", resolved=0x" + std::to_string(static_cast<unsigned long>(resolvedFs.f_type)));
        return;
    }

    if (static_cast<unsigned long>(directFs.f_type) ==
        static_cast<unsigned long>(OVERLAYFS_SUPER_MAGIC)) {
        const bool hasSystemOverlay =
            std::any_of(mounts.begin(), mounts.end(), [](const MountEntry& entry) {
                return (entry.target == "/system" || entry.target == "/system/bin") &&
                       (entry.fsType == "overlay" || entry.fsType == "overlayfs");
            });
        if (!hasSystemOverlay) {
            snapshot.inconsistentMountDetected = true;
            add_finding(snapshot, dedupe, "CONSISTENCY", "DANGER", "Mount consistency",
                        "Hidden overlayfs",
                        "statfs reported overlayfs for /proc/self/exe, but mount tables did not "
                        "show a matching system overlay.");
        }
    }
}
...
MountSnapshot collect_snapshot() {
    ...
    const std::string mountsRaw = read_file_direct("/proc/self/mounts", 131072, &mountsReadable);
    const std::string mountInfoRaw =
        read_file_direct("/proc/self/mountinfo", 196608, &mountInfoReadable);
...
std::string encode_snapshot(const MountSnapshot& snapshot) {
    auto flag = [](bool value) -> const char* { return value ? "1" : "0"; };
    ...
    for (const MountFindingRecord& finding : snapshot.findings) {
        out << "FINDING=" << sanitize_field(finding.group) << '\t'
            << sanitize_field(finding.severity) << '\t' << sanitize_field(finding.label) << '\t'
            << sanitize_field(finding.value) << '\t' << sanitize_field(finding.detail) << '\n';
    }
