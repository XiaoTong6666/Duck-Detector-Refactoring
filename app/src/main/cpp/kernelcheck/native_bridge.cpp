...
std::string read_file_via_syscall(const char* path, size_t max_bytes = 16384) {
    const int fd = static_cast<int>(syscall(__NR_openat, AT_FDCWD, path, O_RDONLY | O_CLOEXEC, 0));
    if (fd < 0) {
        return "";
    }
...
void check_cmdline(KernelSnapshot& snapshot) {
    ...
    for (const auto& check : kCmdlineChecks) {
        if (contains_ignore_case(snapshot.proc_cmdline, check.pattern)) {
            ...
            snapshot.findings.emplace_back(std::string("CMDLINE|") + severity + "|" +
                                           check.description);
        }
    }
    ...
    if (verified_pos != std::string::npos) {
        ...
        const std::string state =
            snapshot.proc_cmdline.substr(value_start, value_end - value_start);
        if (state == "green") {
...
void check_kptr(KernelSnapshot& snapshot) {
    ...
    if (line_count > 0 && non_zero_count > line_count / 2) {
        ...
        finding << "KPTR_RESTRICT|DISABLED|Kernel addresses exposed (" << non_zero_count << "/"
                << line_count << " non-zero)";
