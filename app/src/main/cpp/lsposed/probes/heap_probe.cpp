...
constexpr const char* kKeywords[] = {
    ...
    "xposedbridge",   "handlehookedmethod", "lsplant",
    "libxposed",      "edxposed",           "lspatch",
    "lsposed-bridge", "lsphooker_",
};
...
std::vector<Region> collect_candidate_regions() {
    ...
    while (std::fgets(line, sizeof(line), maps) != nullptr) {
        ...
        char name[512] = {};
        const int parsed =
            std::sscanf(line, "%llx-%llx %4s %*s %*s %*s %511[^\n]", &start, &end, perms, name);
        if (parsed < 3) {
            continue;
        }
        ...
        regions.push_back(Region{
            ...
            .name = region_name.empty() ? "[unnamed dalvik region]" : region_name,
        });
...
bool scan_sample_window(const int mem_fd, const Region& region, const std::uintptr_t offset,
    ...
    const ssize_t bytes_read = ::pread(mem_fd, buffer.data(), window, static_cast<off_t>(absolute));
    if (bytes_read <= 0) {
        return false;
    }
...
bool region_contains_keyword(const int mem_fd, const Region& region) {
