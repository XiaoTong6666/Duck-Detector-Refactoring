...
class KernelCheckCardModelMapper {
    ...
    ): KernelCheckCardModel = KernelCheckCardModel(
        title = "Kernel Check",
    ...
    private fun buildSubtitle(report: KernelCheckReport): String = when (report.stage) {
    ...
    private fun buildVerdict(report: KernelCheckReport): String = when (report.stage) {
    ...
    private fun buildSummary(report: KernelCheckReport): String = when (report.stage) {
    ...
    private fun buildHeaderFacts(report: KernelCheckReport): List<KernelCheckHeaderFactModel> = when (report.stage) {
    ...
    private fun buildIdentityRows(report: KernelCheckReport): List<KernelCheckDetailRowModel> = when (report.stage) {
    ...
    private fun buildAnomalyRows(report: KernelCheckReport): List<KernelCheckDetailRowModel> = when (report.stage) {
        KernelCheckStage.LOADING -> placeholderRows(
    ...
    private fun buildBehaviorRows(report: KernelCheckReport): List<KernelCheckDetailRowModel> = when (report.stage) {
        KernelCheckStage.LOADING -> placeholderRows(
    ...
    private fun buildImpactItems(report: KernelCheckReport): List<KernelCheckImpactItemModel> = when (report.stage) {
    ...
    private fun buildMethodRows(report: KernelCheckReport): List<KernelCheckDetailRowModel> = when (report.stage) {
    ...
    private fun buildScanRows(report: KernelCheckReport): List<KernelCheckDetailRowModel> = when (report.stage) {
        KernelCheckStage.LOADING -> placeholderRows(
            labels = listOf(
        ...
        KernelCheckStage.READY -> listOf(
            ...
            KernelCheckDetailRowModel(
                ...
                status = if (identitySourceCount(report) > 0) {
                    DetectorStatus.allClear()
                } else {
                    DetectorStatus.info(
                        InfoKind.SUPPORT,
    ...
    private fun identityRow(
        label: String,
        value: String,
    ): KernelCheckDetailRowModel = KernelCheckDetailRowModel(
        label = label,
        value = if (value.isNotBlank()) "Captured" else "Unavailable",
        status = if (value.isNotBlank()) {
            DetectorStatus.allClear()
        } else {
            DetectorStatus.info(
                InfoKind.SUPPORT,
    ...
    private fun kptrRow(
        report: KernelCheckReport,
    ): KernelCheckDetailRowModel = when {
        report.kptrExposed -> {
    ...
    ): List<KernelCheckHeaderFactModel> = listOf(
    ...
    ): List<KernelCheckDetailRowModel> = labels.map { label ->
        KernelCheckDetailRowModel(
            label = label,
            value = value,
            status = status,
    ...
    private fun placeholderMethodRows(
        status: DetectorStatus,
        value: String,
    ): List<KernelCheckDetailRowModel> = listOf(
        "emojiScan",
        "chineseScan",
        "scriptScan",
        "telegramScan",
    ...
    private fun identitySourceCount(report: KernelCheckReport): Int = listOf(report.unameOutput, report.procVersion).count { it.isNotBlank() }

    private fun cvePatchStatus(
        state: KernelCheckCvePatchState,
    ): DetectorStatus = when (state) {
        KernelCheckCvePatchState.UNPATCHED,
        KernelCheckCvePatchState.PARTIALLY_PATCHED,
        -> DetectorStatus.info(InfoKind.SUPPORT)
    ...
    private fun methodStatus(result: KernelCheckMethodResult): DetectorStatus = when (result.outcome) {
    ...
    private fun KernelCheckReport.toDetectorStatus(): DetectorStatus = when (stage) {
