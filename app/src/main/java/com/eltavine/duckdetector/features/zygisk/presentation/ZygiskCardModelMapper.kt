...
class ZygiskCardModelMapper {
    ...
    fun map(
        report: ZygiskReport,
    ): ZygiskCardModel = ZygiskCardModel(
        title = "Zygisk",
    ...
    private fun buildHeaderFacts(
        report: ZygiskReport,
    ): List<ZygiskHeaderFactModel> = when (report.stage) {
    ...
    private fun buildStateRows(
        report: ZygiskReport,
    ): List<ZygiskDetailRowModel> = when (report.stage) {
    ...
    private fun buildImpactItems(
        report: ZygiskReport,
    ): List<ZygiskImpactItemModel> = when (report.stage) {
    ...
    private fun signalRow(
        signal: ZygiskSignal,
    ): ZygiskDetailRowModel = ZygiskDetailRowModel(
        label = signal.label,
        value = signal.value,
    ...
    ): List<ZygiskHeaderFactModel> = listOf(
    ...
    private fun placeholderRows(
        ...
        status: DetectorStatus,
        value: String,
    ): List<ZygiskDetailRowModel> = labels.map { label ->
        ZygiskDetailRowModel(
            label = label,
            value = value,
            status = status,
    ...
    ): DetectorStatus = when (method.outcome) {
    ...
    private fun stateLabel(
        report: ZygiskReport,
    ): String = when (report.toDetectorStatus()) {
    ...
    private fun ZygiskReport.toDetectorStatus(): DetectorStatus = when (stage) {
