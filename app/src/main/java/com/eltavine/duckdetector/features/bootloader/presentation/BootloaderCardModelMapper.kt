...
class BootloaderCardModelMapper {
    ...
    ): BootloaderCardModel = BootloaderCardModel(
        title = "Bootloader",
    ...
    private fun buildSubtitle(report: BootloaderReport): String = when (report.stage) {
    ...
    private fun buildVerdict(report: BootloaderReport): String = when (report.stage) {
    ...
    private fun buildSummary(report: BootloaderReport): String = when (report.stage) {
    ...
    private fun buildHeaderFacts(report: BootloaderReport): List<BootloaderHeaderFactModel> = when (report.stage) {
    ...
    ): List<BootloaderDetailRowModel> = when (stage) {
    ...
    private fun buildImpactItems(report: BootloaderReport): List<BootloaderImpactItemModel> = when (report.stage) {
    ...
    private fun buildMethodRows(report: BootloaderReport): List<BootloaderDetailRowModel> = when (report.stage) {
        BootloaderStage.LOADING -> placeholderRows(
            methodPlaceholders(),
            "Pending",
            DetectorStatus.info(InfoKind.SUPPORT),
    ...
    private fun buildScanRows(report: BootloaderReport): List<BootloaderDetailRowModel> = when (report.stage) {
        BootloaderStage.LOADING -> placeholderRows(
            scanPlaceholders(),
            "Pending",
            DetectorStatus.info(InfoKind.SUPPORT),
        ...
        BootloaderStage.READY -> listOf(
            ...
            BootloaderDetailRowModel(
                ...
                status = if (report.observedPropertyCount > 0) {
                    DetectorStatus.allClear()
                } else {
                    DetectorStatus.info(
                        InfoKind.SUPPORT,
            ...
            BootloaderDetailRowModel(
                ...
                status = if (report.nativePropertyHitCount > 0) {
                    DetectorStatus.allClear()
                } else {
                    DetectorStatus.info(
                        InfoKind.SUPPORT,
            ...
            BootloaderDetailRowModel(
                ...
                status = if (report.rawBootParamHitCount > 0) {
                    DetectorStatus.allClear()
                } else {
                    DetectorStatus.info(
                        InfoKind.SUPPORT,
            ...
            BootloaderDetailRowModel(
                ...
                status = if (report.hardwareBacked) {
                    DetectorStatus.allClear()
                } else {
                    DetectorStatus.info(
                        InfoKind.SUPPORT,
    ...
    private fun findingRow(finding: BootloaderFinding): BootloaderDetailRowModel = BootloaderDetailRowModel(
        label = finding.label,
    ...
    ): List<BootloaderHeaderFactModel> = listOf(
    ...
    private fun placeholderRows(
        ...
        value: String,
        status: DetectorStatus,
    ): List<BootloaderDetailRowModel> = labels.map { label ->
        BootloaderDetailRowModel(
            label = label,
            value = value,
            status = status,
    ...
    private fun statePlaceholders(): List<String> = listOf("Boot state", "Evidence source", "Lock state", "Trust root")
    ...
    private fun severityStatus(severity: BootloaderFindingSeverity): DetectorStatus = when (severity) {
    ...
    private fun methodStatus(result: BootloaderMethodResult): DetectorStatus = when (result.outcome) {
    ...
    private fun proofLabel(mode: BootloaderEvidenceMode): String = when (mode) {
    ...
    private fun stateLabel(state: BootloaderState): String = when (state) {
    ...
    private fun tierLabel(tier: TeeTier): String = when (tier) {
    ...
    private fun trustLabel(trustRoot: TeeTrustRoot): String = when (trustRoot) {
    ...
    private fun trustStatus(report: BootloaderReport): DetectorStatus = when {
        report.attestationChainLength == 0 -> DetectorStatus.danger()
    ...
    private fun BootloaderReport.toDetectorStatus(): DetectorStatus = when (stage) {
