...
class BootloaderRepository(
    ...
    private fun buildTrackedProperties(): Map<String, SystemPropertyCategory> = buildMap {
    ...
    private fun buildStateFindings(
        ...
        propertyContext: BootloaderPropertyContext,
    ): List<BootloaderFinding> = buildList {
        add(
            BootloaderFinding(
                id = "boot_state",
                label = "Boot state",
        ...
        add(
            BootloaderFinding(
                ...
                group = BootloaderFindingGroup.STATE,
                severity = when (
                    attestation.rootOfTrust?.deviceLocked
    ...
    ): List<BootloaderImpact> = buildList {
        when (state) {
    ...
    private fun buildMethods(
        ...
        consistencySignals: List<SystemPropertySignal>,
        propertyContext: BootloaderPropertyContext,
    ): List<BootloaderMethodResult> = listOf(
    ...
    private fun propertyLabel(property: String): String = when (property) {
    ...
    private fun propertySeverity(
        ...
        value: String,
        propertyContext: BootloaderPropertyContext,
    ): BootloaderFindingSeverity = when (property) {
        BootloaderCatalog.FLASH_LOCKED,
        BootloaderCatalog.VBMETA_DEVICE_STATE,
        -> when {
    ...
    private fun propertyDetailMonospace(property: String): Boolean = property == BootloaderCatalog.VBMETA_DIGEST ||
        property == BootloaderCatalog.VBMETA_HASH_ALG ||
        property == BootloaderCatalog.AVB_VERSION

    private fun stateSeverity(state: BootloaderState): BootloaderFindingSeverity = when (state) {
    ...
    private fun trustRootSeverity(trust: CertificateTrustResult): BootloaderFindingSeverity = BootloaderSeverityRules.trustRootSeverity(trust)

    private fun trustRootDetail(trust: CertificateTrustResult): String = buildString {
    ...
    private fun buildChainDetail(trust: CertificateTrustResult): String = buildString {
    ...
    private fun evidenceModeLabel(mode: BootloaderEvidenceMode): String = when (mode) {
    ...
    private fun sourceLabel(source: SystemPropertySource): String = when (source) {
    ...
    ): BootloaderFinding = BootloaderFinding(
    ...
    private fun hasAttestation(snapshot: AttestationSnapshot): Boolean = snapshot.rawCertificates.isNotEmpty() || snapshot.rootOfTrust != null
    ...
    private fun isLockedValue(value: String): Boolean = value == "1" || value.equals("locked", ignoreCase = true) || value.equals(
        "true",
        ignoreCase = true,
    )

    private fun isUnlockedValue(value: String): Boolean = value == "0" || value.equals("unlocked", ignoreCase = true) || value.equals(
        "false",
        ignoreCase = true,
    )
    ...
    private data class BootloaderPropertyContext(
        ...
        companion object {
            ...
            fun empty(): BootloaderPropertyContext = BootloaderPropertyContext(
