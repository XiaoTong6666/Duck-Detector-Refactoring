...
class SuCardModelMapper {
    ...
    private fun buildSubtitle(report: SuReport): String = when (report.stage) {
    ...
    private fun buildVerdict(report: SuReport): String = when (report.stage) {
    ...
    private fun buildSummary(report: SuReport): String = when (report.stage) {
        SuStage.LOADING ->
    ...
    private fun buildHeaderFacts(report: SuReport): List<SuHeaderFactModel> = when (report.stage) {
    ...
    private fun buildArtifactRows(report: SuReport): List<SuDetailRowModel> = when (report.stage) {
    ...
    private fun buildContextRows(report: SuReport): List<SuDetailRowModel> = when (report.stage) {
        SuStage.LOADING -> listOf(
            SuDetailRowModel(
                label = "Self context",
                value = "Pending",
    ...
    private fun buildImpactItems(report: SuReport): List<SuImpactItemModel> = when (report.stage) {
        SuStage.LOADING -> listOf(
            SuImpactItemModel(
    ...
    private fun buildMethodRows(report: SuReport): List<SuDetailRowModel> = when (report.stage) {
    ...
    private fun buildScanRows(report: SuReport): List<SuDetailRowModel> = when (report.stage) {
        SuStage.LOADING -> listOf(
            SuDetailRowModel(
                "SU paths checked",
                "Pending",
    ...
    private fun methodStatus(result: SuMethodResult): DetectorStatus = when (result.outcome) {
    ...
    private fun SuReport.toDetectorStatus(): DetectorStatus = when (stage) {
