...
class CustomRomCardModelMapper {
    ...
    ): CustomRomCardModel = CustomRomCardModel(
        title = "Custom ROM",
    ...
    private fun buildSubtitle(report: CustomRomReport): String = when (report.stage) {
    ...
    private fun buildVerdict(report: CustomRomReport): String = when (report.stage) {
    ...
    private fun buildSummary(report: CustomRomReport): String = when (report.stage) {
    ...
    private fun buildHeaderFacts(report: CustomRomReport): List<CustomRomHeaderFactModel> = when (report.stage) {
    ...
    private fun buildBuildRows(report: CustomRomReport): List<CustomRomDetailRowModel> = when (report.stage) {
    ...
    private fun buildRuntimeRows(report: CustomRomReport): List<CustomRomDetailRowModel> = when (report.stage) {
        CustomRomStage.LOADING -> placeholderRows(
    ...
    private fun buildImpactItems(report: CustomRomReport): List<CustomRomImpactItemModel> = when (report.stage) {
    ...
    private fun buildMethodRows(report: CustomRomReport): List<CustomRomDetailRowModel> = when (report.stage) {
        CustomRomStage.LOADING -> placeholderRows(
            labels = listOf(
                "propertyScan",
                "buildFieldScan",
    ...
    private fun buildScanRows(report: CustomRomReport): List<CustomRomDetailRowModel> = when (report.stage) {
        CustomRomStage.LOADING -> placeholderRows(
            labels = listOf(
                "Properties checked",
    ...
    ): CustomRomDetailRowModel = CustomRomDetailRowModel(
    ...
    ): List<CustomRomHeaderFactModel> = listOf(
    ...
    private fun placeholderRows(
        ...
        status: DetectorStatus,
        value: String,
    ): List<CustomRomDetailRowModel> = labels.map { label ->
        CustomRomDetailRowModel(
            label = label,
            value = value,
            status = status,
    ...
    private fun detectedRomValue(report: CustomRomReport): String = when {
    ...
    private fun signalValue(count: Int): String = if (count > 0) count.toString() else "None"

    private fun buildCleanModificationDetail(report: CustomRomReport): String = buildString {
    ...
    private fun methodStatus(result: CustomRomMethodResult): DetectorStatus = when (result.outcome) {
    ...
    private fun CustomRomReport.toDetectorStatus(): DetectorStatus = when (stage) {
    ...
    private fun CustomRomReport.hasReducedCoverage(): Boolean = !nativeAvailable ||
