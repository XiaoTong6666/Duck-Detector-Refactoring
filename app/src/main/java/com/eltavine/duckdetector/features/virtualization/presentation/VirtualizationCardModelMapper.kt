...
class VirtualizationCardModelMapper {
    ...
    fun map(report: VirtualizationReport): VirtualizationCardModel = VirtualizationCardModel(
    ...
    private fun buildSubtitle(report: VirtualizationReport): String = when (report.stage) {
    ...
    private fun buildVerdict(report: VirtualizationReport): String = when (report.stage) {
    ...
    private fun buildSummary(report: VirtualizationReport): String = when (report.stage) {
    ...
    private fun buildHeaderFacts(report: VirtualizationReport): List<VirtualizationHeaderFactModel> = when (report.stage) {
    ...
    ): List<VirtualizationDetailRowModel> = when (stage) {
    ...
    private fun buildImpactItems(report: VirtualizationReport): List<VirtualizationImpactItemModel> = when (report.stage) {
    ...
    private fun buildMethodRows(report: VirtualizationReport): List<VirtualizationDetailRowModel> = when (report.stage) {
        VirtualizationStage.LOADING -> placeholderRows(
            METHOD_LABELS,
            "Pending",
            DetectorStatus.info(InfoKind.SUPPORT),
    ...
    private fun buildScanRows(report: VirtualizationReport): List<VirtualizationDetailRowModel> = when (report.stage) {
        VirtualizationStage.LOADING -> placeholderRows(
            SCAN_LABELS,
            "Pending",
            DetectorStatus.info(InfoKind.SUPPORT),
        ...
        VirtualizationStage.READY -> listOf(
            VirtualizationDetailRowModel(
                ...
                status = if (report.startupPreloadAvailable) {
                    DetectorStatus.allClear()
                } else {
                    DetectorStatus.info(
                        InfoKind.SUPPORT,
            ...
            VirtualizationDetailRowModel(
                ...
                status = if (report.startupPreloadContextValid) {
                    DetectorStatus.allClear()
                } else {
                    DetectorStatus.info(
                        InfoKind.SUPPORT,
            ...
            VirtualizationDetailRowModel(
                ...
                status = if (report.crossProcessAvailable) {
                    DetectorStatus.allClear()
                } else {
                    DetectorStatus.info(
                        InfoKind.SUPPORT,
            ...
            VirtualizationDetailRowModel(
                ...
                status = if (report.isolatedProcessAvailable) {
                    DetectorStatus.allClear()
                } else {
                    DetectorStatus.info(
                        InfoKind.SUPPORT,
            ...
            VirtualizationDetailRowModel(
                ...
                status = if (report.mountNamespaceAvailable) {
                    DetectorStatus.allClear()
                } else {
                    DetectorStatus.info(
                        InfoKind.SUPPORT,
            ...
            VirtualizationDetailRowModel(
                ...
                status = if (report.syscallPackSupported) {
                    DetectorStatus.allClear()
                } else {
                    DetectorStatus.info(
                        InfoKind.SUPPORT,
    ...
    private fun signalRow(signal: VirtualizationSignal): VirtualizationDetailRowModel = VirtualizationDetailRowModel(
        label = signal.label,
        value = signal.value,
    ...
    private fun VirtualizationSignal.toStatus(): DetectorStatus = when (severity) {
    ...
    private fun VirtualizationMethodResult.toStatus(): DetectorStatus = when (outcome) {
    ...
    private fun VirtualizationImpact.toStatus(): DetectorStatus = when (severity) {
        VirtualizationSignalSeverity.DANGER -> DetectorStatus.danger()
        VirtualizationSignalSeverity.WARNING -> DetectorStatus.warning()
        VirtualizationSignalSeverity.INFO -> DetectorStatus.info(InfoKind.SUPPORT)
        VirtualizationSignalSeverity.SAFE -> DetectorStatus.allClear()
    ...
    private fun VirtualizationReport.toDetectorStatus(): DetectorStatus = when (stage) {
    ...
    private fun VirtualizationReport.hasReducedCoverage(): Boolean = !nativeAvailable ||
    ...
    ): List<VirtualizationDetailRowModel> = labels.map { label ->
    ...
    private fun placeholderFacts(
        value: String,
        status: DetectorStatus,
    ): List<VirtualizationHeaderFactModel> = listOf(
    ...
    private fun severityStatus(count: Int): DetectorStatus = if (count > 0) DetectorStatus.warning() else DetectorStatus.allClear()
