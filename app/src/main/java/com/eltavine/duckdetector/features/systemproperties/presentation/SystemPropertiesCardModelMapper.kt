...
class SystemPropertiesCardModelMapper {
    ...
    ): SystemPropertiesCardModel = SystemPropertiesCardModel(
    ...
    private fun buildSubtitle(report: SystemPropertiesReport): String = when (report.stage) {
    ...
    private fun buildVerdict(report: SystemPropertiesReport): String = when (report.stage) {
    ...
    private fun buildSummary(report: SystemPropertiesReport): String = when (report.stage) {
    ...
    private fun buildHeaderFacts(report: SystemPropertiesReport): List<SystemPropertiesHeaderFactModel> = when (report.stage) {
    ...
    private fun buildCoreRows(report: SystemPropertiesReport): List<SystemPropertiesDetailRowModel> = when (report.stage) {
    ...
    private fun buildBootRows(report: SystemPropertiesReport): List<SystemPropertiesDetailRowModel> = when (report.stage) {
        SystemPropertiesStage.LOADING -> placeholderRows(
            labels = listOf(
    ...
    private fun buildBuildRows(report: SystemPropertiesReport): List<SystemPropertiesDetailRowModel> = when (report.stage) {
        SystemPropertiesStage.LOADING -> placeholderRows(
            labels = listOf(
                "ro.build.type",
                "ro.build.tags",
    ...
    private fun buildSourceRows(report: SystemPropertiesReport): List<SystemPropertiesDetailRowModel> = when (report.stage) {
        SystemPropertiesStage.LOADING -> placeholderRows(
    ...
    private fun buildConsistencyRows(report: SystemPropertiesReport): List<SystemPropertiesDetailRowModel> = when (report.stage) {
        SystemPropertiesStage.LOADING -> placeholderRows(
            labels = listOf(
    ...
    private fun buildInfoRows(report: SystemPropertiesReport): List<SystemPropertiesDetailRowModel> = when (report.stage) {
        SystemPropertiesStage.LOADING -> placeholderRows(
            labels = listOf(
                "ro.product.model",
    ...
    private fun buildImpactItems(report: SystemPropertiesReport): List<SystemPropertiesImpactItemModel> = when (report.stage) {
    ...
    private fun buildMethodRows(report: SystemPropertiesReport): List<SystemPropertiesDetailRowModel> = when (report.stage) {
    ...
    private fun buildScanRows(report: SystemPropertiesReport): List<SystemPropertiesDetailRowModel> = when (report.stage) {
        SystemPropertiesStage.LOADING -> placeholderRows(
            labels = listOf(
                "Rules checked",
                "Rules observed",
        ...
        SystemPropertiesStage.READY -> run {
            ...
            listOf(
                ...
                SystemPropertiesDetailRowModel(
                    ...
                    status = if (report.observedRuleCount > 0) {
                ...
                SystemPropertiesDetailRowModel(
                    ...
                    status = if (report.reflectionHitCount > 0) {
                        DetectorStatus.allClear()
                    } else {
                        DetectorStatus.info(
                            InfoKind.SUPPORT,
                ...
                SystemPropertiesDetailRowModel(
                    ...
                    status = if (report.getpropHitCount > 0) {
                        DetectorStatus.allClear()
                    } else {
                        DetectorStatus.info(
                            InfoKind.SUPPORT,
                ...
                SystemPropertiesDetailRowModel(
                    ...
                    status = if (report.nativeHitCount > 0) {
                        DetectorStatus.allClear()
                    } else {
                        DetectorStatus.info(
                            InfoKind.SUPPORT,
                ...
                SystemPropertiesDetailRowModel(
                    ...
                    status = if (report.bootParamHitCount > 0) {
                        DetectorStatus.allClear()
                    } else {
                        DetectorStatus.info(
                            InfoKind.SUPPORT,
                ...
                SystemPropertiesDetailRowModel(
                    ...
                    status = if (report.propAreaAvailable) {
                        DetectorStatus.allClear()
                    } else {
                        DetectorStatus.info(
                            InfoKind.SUPPORT,
                ...
                SystemPropertiesDetailRowModel(
                    ...
                    status = if (report.infoPropertyCount > 0) {
                        DetectorStatus.allClear()
                    } else {
                        DetectorStatus.info(
                            InfoKind.SUPPORT,
    ...
    private fun infoRow(signal: SystemPropertySignal): SystemPropertiesDetailRowModel = SystemPropertiesDetailRowModel(
    ...
    ): List<SystemPropertiesHeaderFactModel> = listOf(
    ...
    ): List<SystemPropertiesDetailRowModel> = labels.map { label ->
        SystemPropertiesDetailRowModel(
            label = label,
            value = value,
            status = status,
    ...
    private fun placeholderMethodRows(
        status: DetectorStatus,
        value: String,
    ): List<SystemPropertiesDetailRowModel> = listOf(
        "Reflection API",
        "getprop snapshot",
    ...
    private fun countLabel(count: Int): String = if (count > 0) count.toString() else "None"

    private fun badgeValue(
        value: String,
    ): String = if (value.length > MAX_BADGE_LENGTH) {
    ...
    ): DetectorStatus = when (signal.severity) {
    ...
    ): DetectorStatus = when (result.outcome) {
    ...
    private fun SystemPropertiesReport.toDetectorStatus(): DetectorStatus = when (stage) {
    ...
    private fun SystemPropertiesReport.hasReducedCoverage(): Boolean = !propAreaAvailable

    private fun isPropAreaHoleSignal(
        signal: SystemPropertySignal,
    ): Boolean = signal.property.startsWith("prop_area hole:")

    private fun isCrossCheckSignal(
        signal: SystemPropertySignal,
    ): Boolean = signal.category == SystemPropertyCategory.PROPERTY_CONSISTENCY &&
