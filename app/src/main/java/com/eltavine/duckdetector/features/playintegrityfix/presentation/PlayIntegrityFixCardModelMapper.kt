...
class PlayIntegrityFixCardModelMapper {
    ...
    ): PlayIntegrityFixCardModel = PlayIntegrityFixCardModel(
    ...
    private fun buildSubtitle(report: PlayIntegrityFixReport): String = when (report.stage) {
    ...
    private fun buildVerdict(report: PlayIntegrityFixReport): String = when (report.stage) {
    ...
    private fun buildSummary(report: PlayIntegrityFixReport): String = when (report.stage) {
    ...
    private fun buildHeaderFacts(report: PlayIntegrityFixReport): List<PlayIntegrityFixHeaderFactModel> = when (report.stage) {
    ...
    private fun buildPropertyRows(report: PlayIntegrityFixReport): List<PlayIntegrityFixDetailRowModel> = when (report.stage) {
        ...
        PlayIntegrityFixStage.READY ->
            report.propertySignals
                .sortedWith(compareBy<PlayIntegrityFixSignal> { it.category.ordinal }.thenBy { it.label })
                .map(::signalRow)
    }

    private fun buildConsistencyRows(report: PlayIntegrityFixReport): List<PlayIntegrityFixDetailRowModel> = when (report.stage) {
        PlayIntegrityFixStage.LOADING -> placeholderRows(
        ...
        PlayIntegrityFixStage.READY ->
            report.consistencySignals
                .sortedBy { it.label }
                .map(::signalRow)
    }

    private fun buildNativeRows(report: PlayIntegrityFixReport): List<PlayIntegrityFixDetailRowModel> = when (report.stage) {
        PlayIntegrityFixStage.LOADING -> placeholderRows(
    ...
    private fun buildImpactItems(report: PlayIntegrityFixReport): List<PlayIntegrityFixImpactItemModel> = when (report.stage) {
    ...
    private fun buildMethodRows(report: PlayIntegrityFixReport): List<PlayIntegrityFixDetailRowModel> = when (report.stage) {
    ...
    private fun buildScanRows(report: PlayIntegrityFixReport): List<PlayIntegrityFixDetailRowModel> = when (report.stage) {
        PlayIntegrityFixStage.LOADING -> placeholderRows(
            labels = listOf(
                "Properties checked",
                "Property hits",
    ...
    ): List<PlayIntegrityFixHeaderFactModel> = listOf(
    ...
    ): List<PlayIntegrityFixDetailRowModel> = labels.map { label ->
        PlayIntegrityFixDetailRowModel(
            label = label,
            value = value,
            status = status,
    ...
    private fun placeholderMethodRows(
        status: DetectorStatus,
        value: String,
    ): List<PlayIntegrityFixDetailRowModel> = listOf(
        "Reflection API",
        "getprop snapshot",
        "JVM fallback",
        "Native libc props",
    ...
    private fun propertyStatus(report: PlayIntegrityFixReport): DetectorStatus = when {
    ...
    private fun nativeFactStatus(report: PlayIntegrityFixReport): DetectorStatus = when {
        report.nativeSignals.any { it.severity == PlayIntegrityFixSignalSeverity.DANGER } -> DetectorStatus.danger()
    ...
    private fun countOrNone(count: Int): String = if (count > 0) count.toString() else "None"

    private fun countOrClean(count: Int): String = if (count > 0) count.toString() else "Clean"

    private fun badgeValue(value: String): String = if (value.length > MAX_BADGE_LENGTH) {
        value.take(MAX_BADGE_LENGTH - 1) + "…"
    } else {
        value
    }

    private fun signalStatus(signal: PlayIntegrityFixSignal): DetectorStatus = when (signal.severity) {
        PlayIntegrityFixSignalSeverity.DANGER -> DetectorStatus.danger()
        PlayIntegrityFixSignalSeverity.WARNING -> DetectorStatus.warning()
    }

    private fun methodStatus(result: PlayIntegrityFixMethodResult): DetectorStatus = when (result.outcome) {
    ...
    private fun PlayIntegrityFixReport.toDetectorStatus(): DetectorStatus = when (stage) {
