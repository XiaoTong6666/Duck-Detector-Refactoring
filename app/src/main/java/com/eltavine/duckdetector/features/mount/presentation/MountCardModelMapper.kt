...
class MountCardModelMapper {
    ...
    private fun buildSubtitle(report: MountReport): String = when (report.stage) {
    ...
    private fun buildVerdict(report: MountReport): String = when (report.stage) {
    ...
    private fun buildSummary(report: MountReport): String = when (report.stage) {
        MountStage.LOADING ->
    ...
    private fun buildHeaderFacts(report: MountReport): List<MountHeaderFactModel> = when (report.stage) {
    ...
    ): List<MountDetailRowModel> = when (stage) {
    ...
    private fun buildImpactItems(report: MountReport): List<MountImpactItemModel> = when (report.stage) {
    ...
    private fun buildMethodRows(report: MountReport): List<MountDetailRowModel> = when (report.stage) {
        MountStage.LOADING -> placeholderRows(
            methodLabels(),
            "Pending",
            DetectorStatus.info(InfoKind.SUPPORT),
    ...
    private fun buildScanRows(report: MountReport): List<MountDetailRowModel> = when (report.stage) {
        MountStage.LOADING -> placeholderRows(
            scanLabels(),
            "Pending",
            DetectorStatus.info(InfoKind.SUPPORT),
    ...
    private fun findingRow(finding: MountFinding): MountDetailRowModel = MountDetailRowModel(
        label = finding.label,
    ...
    ): List<MountHeaderFactModel> = listOf(
    ...
    private fun placeholderRows(
        ...
        value: String,
        status: DetectorStatus,
    ): List<MountDetailRowModel> = labels.map { label ->
        MountDetailRowModel(
            label = label,
            value = value,
            status = status,
    ...
    private fun severityStatus(severity: MountFindingSeverity): DetectorStatus = when (severity) {
    ...
    private fun methodStatus(result: MountMethodResult): DetectorStatus = when (result.outcome) {
    ...
    private fun boolStatus(value: Boolean): DetectorStatus = if (value) DetectorStatus.allClear() else DetectorStatus.info(InfoKind.SUPPORT)

    private fun coveragePercent(report: MountReport): Int = if (report.permissionTotal <= 0) {
        100
    } else {
        ((report.permissionAccessible.toDouble() / report.permissionTotal.toDouble()) * 100.0).toInt()
    }

    private fun badgeValue(value: String): String = if (value.length > 18) value.take(17) + "…" else value

    private fun preloadStateLabel(report: MountReport): String = when {
        !report.earlyPreloadAvailable -> "Unavailable"
        report.earlyPreloadDetected -> "Detected"
        else -> "Clean"
    }

    private fun preloadContextLabel(report: MountReport): String = when {
        !report.earlyPreloadAvailable -> "N/A"
        report.earlyPreloadContextValid -> "Fresh"
        else -> "Stale"
    }

    private fun preloadStatus(report: MountReport): DetectorStatus = when {
    ...
    private fun preloadContextStatus(report: MountReport): DetectorStatus = when {
        !report.earlyPreloadAvailable -> DetectorStatus.info(InfoKind.SUPPORT)
        report.earlyPreloadContextValid -> DetectorStatus.allClear()
        else -> DetectorStatus.info(InfoKind.SUPPORT)
    }

    private fun hasDangerPreloadEvidence(report: MountReport): Boolean = report.findings.any { finding ->
        (
            finding.id.startsWith("early_preload_") ||
                finding.detail.orEmpty()
                    .contains("Startup preload:")
            ) &&
            finding.severity == MountFindingSeverity.DANGER
    }

    private fun hasWarningPreloadEvidence(report: MountReport): Boolean = report.findings.any { finding ->
    ...
    private fun MountReport.toDetectorStatus(): DetectorStatus = when (stage) {
