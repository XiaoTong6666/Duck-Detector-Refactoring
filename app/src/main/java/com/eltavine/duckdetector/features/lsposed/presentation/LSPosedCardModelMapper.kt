...
class LSPosedCardModelMapper {
    ...
    fun map(report: LSPosedReport): LSPosedCardModel = LSPosedCardModel(
        title = "LSPosed",
    ...
    private fun buildSubtitle(report: LSPosedReport): String = when (report.stage) {
    ...
    private fun buildVerdict(report: LSPosedReport): String = when (report.stage) {
    ...
    private fun buildSummary(report: LSPosedReport): String = when (report.stage) {
    ...
    private fun buildHeaderFacts(report: LSPosedReport): List<LSPosedHeaderFactModel> = when (report.stage) {
    ...
    ): List<LSPosedDetailRowModel> = when (report.stage) {
    ...
    private fun buildImpactItems(report: LSPosedReport): List<LSPosedImpactItemModel> = when (report.stage) {
    ...
    private fun buildMethodRows(report: LSPosedReport): List<LSPosedDetailRowModel> = when (report.stage) {
    ...
    private fun buildScanRows(report: LSPosedReport): List<LSPosedDetailRowModel> = when (report.stage) {
        LSPosedStage.LOADING -> placeholderRows(
            listOf(
                "Danger signals",
                "Review signals",
        ...
        LSPosedStage.READY -> listOf(
            ...
            LSPosedDetailRowModel(
                ...
                status = if (report.runtimeArtifactAvailable) {
            ...
            LSPosedDetailRowModel(
                ...
                status = if (report.logcatAvailable) {
                    DetectorStatus.allClear()
                } else {
                    DetectorStatus.info(
                        InfoKind.SUPPORT,
            ...
            LSPosedDetailRowModel(
                ...
                status = if (report.dirtyPolicyAvailable) {
                    DetectorStatus.allClear()
                } else {
                    DetectorStatus.info(
                        InfoKind.SUPPORT,
    ...
    private fun signalRow(signal: LSPosedSignal): LSPosedDetailRowModel = LSPosedDetailRowModel(
        label = signal.label,
        value = signal.value,
    ...
    ): List<LSPosedHeaderFactModel> = listOf(
    ...
    private fun placeholderRows(
        ...
        status: DetectorStatus,
        value: String,
    ): List<LSPosedDetailRowModel> = labels.map { label ->
        LSPosedDetailRowModel(
            label = label,
            value = value,
            status = status,
    ...
    private fun placeholderMethodRows(
        status: DetectorStatus,
        value: String,
    ): List<LSPosedDetailRowModel> = listOf(
        "Class load",
        "ClassLoader chain",
        "XposedBridge fields",
        "Package catalog",
    ...
    private fun countLabel(count: Int, reducedCoverage: Boolean = false): String = when {
    ...
    private fun signalStatus(signal: LSPosedSignal): DetectorStatus = when (signal.severity) {
        LSPosedSignalSeverity.DANGER -> DetectorStatus.danger()
        LSPosedSignalSeverity.WARNING -> DetectorStatus.warning()
    }

    private fun methodStatus(method: LSPosedMethodResult): DetectorStatus = when (method.outcome) {
    ...
    private fun LSPosedReport.toDetectorStatus(): DetectorStatus = when (stage) {
    ...
    private fun LSPosedReport.hasReducedCoverage(): Boolean = !nativeAvailable ||
        !nativeHeapAvailable ||
    ...
    private fun LSPosedReport.sectionUnavailable(group: LSPosedSignalGroup): Boolean = when (group) {
    ...
    private fun LSPosedReport.hasPolicySignals(): Boolean = policySignalCount > 0

    private fun LSPosedReport.hasLsposedPolicySignal(): Boolean = signals.any {
        it.group == LSPosedSignalGroup.POLICY &&
